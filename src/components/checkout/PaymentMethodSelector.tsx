...
const OPTIONS: Option[] = [
  {
    key: 'stripe',
    label: 'Stripe',
    description: 'Kredi / Banka Kartı (EU/UK/US)',
    icon: CreditCard,
  ...
  },
  {
    key: 'iyzico',
    label: 'iyzico',
    description: 'Kredi Kartı / Taksit (Türkiye)',
    icon: CreditCard,
    regions: ['TR'],
  },
  {
...
export function PaymentMethodSelector({ selected, onChange, region }: PaymentMethodSelectorProps) {
  const available = OPTIONS.filter(
    (o) => o.regions.includes(region) || o.regions.includes('GLOBAL'),
  );
            ...
            <div
              className={cn(
                ...
                selected === opt.key
                  ...
                  : 'bg-white text-brand-primary/40 border border-brand-primary/10',
              )}
            >
              <opt.icon size={20} />
            </div>
            <div className="flex-1 min-w-0">
              <p
                className={cn(
                  ...
                  selected === opt.key ? 'text-accent' : 'text-brand-primary',
                )}
              >
                {opt.label}
              </p>
              <p className="text-[10px] font-medium text-brand-primary/50">{opt.description}</p>
            </div>
            {selected === opt.key && (

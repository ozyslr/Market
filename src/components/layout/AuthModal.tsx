        ...
        <>
          ...
          >
            <div className="relative bg-gradient-to-br from-brand-primary to-zinc-800 p-8 pb-6 text-white overflow-hidden">
              ...
              <button
                onClick={onClose}
                className="absolute top-5 end-5 p-2 bg-white/10 rounded-full hover:bg-white/20 transition-colors z-10"
                aria-label="Kapat"
              >
                <X size={16} />
              </button>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-4">
              {mode === 'register' && (
                <div className="relative">
                  <User
                    size={16}
                    className="absolute start-4 top-1/2 -translate-y-1/2 text-brand-primary/30"
                  />
                  <input
                    type="text"
                    placeholder={t('form.fullname')}
                    aria-label={t('form.fullname') || 'Ad Soyad'}
                    value={name}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  aria-label={t('form.email') || 'E-posta'}
                  value={email}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  aria-label={t('form.password') || 'Şifre'}
                  value={password}
                ...
                <button
                  ...
                  className="absolute end-4 top-1/2 -translate-y-1/2 text-brand-primary/30 hover:text-brand-primary transition-colors"
                >
              ...
              {error && (
                <p
                  className="text-xs font-bold text-red-500 bg-red-50 dark:bg-red-900/20 px-4 py-2.5 rounded-xl"
                  role="alert"
                  aria-live="assertive"
                >
                  {error}
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (code === 'auth/popup-closed-by-user') {
                      // User closed the popup — not an error to surface loudly
                      console.log('Google sign-in popup closed by user');
                    } else if (code === 'auth/popup-blocked') {
                      setError(
                        t('auth.error.popupBlocked') ||
                          'Lütfen pop-up engelleyiciyi devre dışı bırakıp tekrar deneyin.',
                      );
                    } else if (code === 'auth/cancelled-popup-request') {
                      // User clicked away — ignore
                    } else if (code === 'auth/unauthorized-domain') {
                      setError(
                        "Bu alan adı Firebase'de yetkilendirilmemiş. Lütfen yöneticiyle iletişime geçin.",
                      );
                    } else {
                      setError(
                        t('auth.error.generic') || 'Google ile giriş yapılırken bir hata oluştu.',
                      );
                ...
                className="w-full py-3.5 border border-brand-primary/10 dark:border-white/10 rounded-2xl font-bold text-sm text-brand-primary dark:text-white hover:bg-brand-secondary/50 dark:hover:bg-zinc-900 transition-all flex items-center justify-center gap-3 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {googleLoading ? (

...
export function Navbar() {
  ...
  const favoriteBadge =
    favoriteCount > 30 ? '30+' : favoriteCount > 0 ? String(favoriteCount) : null;
      ...
      >
        <div className="max-w-[1700px] mx-auto px-4 lg:px-8 flex items-center gap-8">
          ...
          <Link
            ...
            aria-label="Benim Olan ana sayfa"
          >
            <img
              src="/logo.png"
              alt="Benim Olan"
          ...
          <div className="flex sm:hidden items-center gap-1 md:gap-3 ms-auto">
            <div className="flex items-center gap-1 xl:gap-2 me-2">
              <div
                ...
                aria-label={t('nav.language') || 'Dil'}
              >
                <Globe size={20} className="text-brand-primary dark:text-white" />
                <span className="absolute -top-1 -end-1 bg-accent text-white text-[8px] font-black w-4 h-4 rounded-full flex items-center justify-center border-2 border-brand-primary shadow-lg">
                  {lang.toUpperCase()}
                </span>
              </div>
            </div>
            ...
            >
              <div className="flex items-center gap-1 opacity-60">
                ...
                <span className="text-[8px] font-black uppercase tracking-widest">
                  {t('nav.location_select')}
                </span>
                <ChevronDown size={10} />
              </div>
              <div className="text-[10px] font-black uppercase tracking-tighter truncate">
                {selectedLocation}
              </div>
            </div>
            <SearchBar />
          ...
          <div className="flex items-center gap-4 lg:gap-6 shrink-0 ms-auto">
            ...
            >
              ...
              >
                ...
                <span className="absolute -top-1 -end-1 bg-accent text-white text-[8px] font-black w-4 h-4 rounded-full flex items-center justify-center shadow-lg shadow-accent/20 uppercase">
                  {lang}
                </span>
              </button>
              {isLangMenuOpen && (
                  ...
                  {availableLanguages &&
                    availableLanguages.map((l) => (
                      <button
                        ...
                        role="menuitem"
                        className={cn(
                          'w-full text-start px-3 py-2.5 rounded-xl text-xs font-bold transition-colors flex items-center justify-between group/lang',
                          lang === l.code
                            ? 'bg-accent/10 text-accent'
                            : 'hover:bg-brand-secondary dark:hover:bg-zinc-900 text-brand-primary dark:text-white',
                        )}
                      >
            ...
            <div
              ...
              onBlur={(e) => {
                if (!e.currentTarget.contains(e.relatedTarget)) setIsAccountMenuOpen(false);
              }}
            >
              <button
                ...
                >
                  {user ? (
                    <div className="flex flex-col gap-6">
                      <Link
                        to="/profile"
                        className="flex items-center gap-5 p-5 bg-gradient-to-br from-brand-secondary/50 to-white dark:from-zinc-900 dark:to-zinc-950 border border-brand-primary/5 dark:border-white/5 rounded-3xl relative overflow-hidden hover:opacity-90 transition-opacity cursor-pointer"
                      >
                        ...
                        {user.photoURL ? (
                          <img
                            src={user.photoURL}
                            alt={user.name}
                            className="w-16 h-16 rounded-3xl object-cover shadow-[0_10px_20px_-10px_rgba(249,66,58,0.5)] relative z-10 shrink-0"
                        ...
                        <div className="flex-1 relative z-10">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="px-2 py-0.5 bg-brand-primary text-white text-[8px] font-black uppercase tracking-widest rounded-lg">
                              Elite
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] font-bold text-brand-primary/60">
                              0 Puan
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 rounded-2xl bg-brand-primary text-white group/stat cursor-pointer">
                          <div className="flex justify-between items-start mb-2">
                            <Wallet
                              size={16}
                              className="text-accent group-hover/stat:rotate-12 transition-transform"
                            />
                            <ArrowUpRight size={14} className="text-white/40" />
                          </div>
                          <span className="text-[10px] font-black uppercase tracking-widest text-white/60 block mb-1">
                            Toplam Harcama
                          </span>
                          <span className="text-xl font-display font-black">
                            {user.currency === 'GBP' ? '£' : user.currency === 'USD' ? '$' : '₺'}
                            {(user.spentTotal ?? 0).toLocaleString('tr-TR', {
                        ...
                        <Link
                          ...
                          className="p-4 rounded-2xl border border-brand-primary/10 dark:border-white/10 hover:border-accent hover:bg-accent/5 transition-colors group/stat flex flex-col justify-between"
                        >
                          <div className="flex justify-between items-start mb-2">
                            <Package
                              size={16}
                              className="text-brand-primary dark:text-white group-hover/stat:text-accent transition-colors"
                            />
                            {activeOrdersCount !== null && activeOrdersCount > 0 && (
                              <span className="w-5 h-5 bg-accent text-white rounded-full flex items-center justify-center text-[9px] font-bold">
                                {activeOrdersCount > 9 ? '9+' : activeOrdersCount}
                              </span>
                            )}
                          </div>
                          <span className="text-[10px] font-black uppercase tracking-widest text-brand-primary/40 block mb-1">
                            Aktif Sipariş
                          </span>
                      ...
                      <div className="grid grid-cols-2 gap-x-8 gap-y-6 pt-2 border-t border-brand-primary/5">
                        <div className="flex flex-col gap-3">
                          ...
                          <Link
                            to="/orders"
                            className="text-xs font-bold hover:text-mercora-red transition-colors py-1 flex items-center gap-2 group/link"
                          >
                            <ShoppingBag
                              size={12}
                              className="text-accent/70 group-hover/link:text-mercora-red transition-colors shrink-0"
                            />{' '}
                            Siparişlerim
                          </Link>
                          <Link
                            ...
                            className="text-xs font-bold hover:text-mercora-red transition-colors py-1 flex items-center gap-2 group/link"
                          >
                            <span className="w-1.5 h-1.5 rounded-full bg-transparent group-hover/link:bg-mercora-red transition-colors" />{' '}
                            Tüm Siparişlerim
                          </Link>
                          <Link
                      ...
                      <div className="flex flex-col gap-2 mt-2 bg-brand-secondary/30 dark:bg-zinc-800/30 p-4 rounded-2xl border border-brand-primary/5">
                        <span className="text-[10px] uppercase font-black tracking-[0.2em] text-brand-primary/60 dark:text-white/60 mb-2">
                          Uzman Portalları
                        </span>
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          {user.role === 'moderator' && (
                            <Link
                              to="/moderator"
                              className="p-3 rounded-xl bg-purple-50 dark:bg-purple-900/20 text-purple-600 hover:scale-105 transition-transform flex items-center gap-3"
                            >
                      ...
                      <div className="border-t border-brand-primary/5 dark:border-white/5 pt-4 flex items-center gap-4">
                        <Link
                          ...
                          className="flex-1 py-3 text-center bg-zinc-50 dark:bg-zinc-900 text-brand-primary dark:text-white rounded-xl font-bold text-xs hover:bg-zinc-100 transition-colors"
                        >
                          Ayarlar
                        </Link>
                        <button
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="bg-gradient-to-r from-mercora-red to-orange-500 rounded-xl p-4 text-white">
                        ...
                        <p className="text-xs opacity-90 mb-4">
                          Size özel kampanyalardan ve indirimlerden faydalanın.
                        </p>
                        <button
                          onClick={() => openAuth('login')}
                          className="w-full py-3 bg-white dark:bg-zinc-800 text-mercora-red rounded-xl font-black text-sm shadow-md hover:scale-105 transition-transform"
                        >
                          Giriş Yap
                        </button>
                      </div>
                      <button
                        ...
                        className="w-full py-3 bg-zinc-100 dark:bg-zinc-900 text-brand-primary dark:text-white border border-brand-primary/10 dark:border-zinc-800 rounded-xl font-bold text-sm hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors"
                      >
                        Üye Ol
                      </button>
            ...
            <Link
              to="/cart"
              className="relative group p-2 hover:bg-mercora-red/5 rounded-lg flex items-center gap-2 transition-colors"
            ...
            >
              <div className="relative text-brand-primary dark:text-white group-hover:text-mercora-red transition-colors">
                ...
                />
                {itemCount > 0 && (
                  <span className="absolute -top-1.5 -end-1.5 w-4 h-4 bg-mercora-red text-white rounded-full text-[10px] font-bold flex items-center justify-center shadow-sm">
                    {itemCount}
                  </span>
                )}
              </div>
              <div className="hidden lg:block leading-none">
                <span className="text-xs font-bold text-brand-primary dark:text-white whitespace-nowrap group-hover:text-mercora-red transition-colors">
                  Sepetim
                </span>
              </div>
            </Link>
        ...
        <div className="px-4 pb-3 sm:hidden transition-all duration-300 bg-white dark:bg-zinc-950">
          ...
          >
            ...
            <button
              type="submit"
              className="absolute end-0 h-full px-4 text-accent"
              aria-label="Ara"
            >
      ...
      >
        <div className="max-w-[1700px] mx-auto px-4 lg:px-8 flex items-center justify-between">
          <div className="flex items-center gap-8 overflow-x-auto no-scrollbar whitespace-nowrap">
            ...
            {[
              ...
              <Link
                key={i}
                to={item.path}
                className="text-[10px] font-black uppercase tracking-widest hover:text-accent transition-all hidden md:block opacity-60 hover:opacity-100"
              >
                {item.name}
              </Link>
            ))}
          ...
          <div className="hidden xl:flex items-center gap-6">
            <Link
              to="/sell"
              className="text-[10px] font-black uppercase tracking-widest text-accent flex items-center gap-2 group"
            >

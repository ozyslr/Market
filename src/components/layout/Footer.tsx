    ...
    <footer className="bg-white dark:bg-zinc-950 text-brand-primary dark:text-white pt-16 pb-8 border-t border-brand-primary/5">
      <div className="max-w-[1700px] mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 pb-12 mb-12 border-b border-brand-primary/5">
           ...
           <div className="flex flex-col items-center text-center gap-3">
              <div className="w-16 h-16 rounded-full bg-[#F9423A]/10 flex items-center justify-center text-[#F9423A]">
                 <Zap size={32} />
              </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-8 mb-16 px-4">
          ...
          <div className="xl:col-span-1 hidden xl:block">
            <Link to="/" className="flex items-center gap-2 mb-6 group w-fit">
              <div className="w-10 h-10 bg-[#F9423A] rounded-xl flex items-center justify-center text-white scale-100 group-hover:scale-110 transition-transform shadow-lg">
                <ShoppingBagIcon />
              </div>
            ...
            <div key={i}>
              ...
              <ul className="space-y-4">
                {section.links.map(link => (
                  <li key={link}>
                    <Link to="/" className="text-brand-primary/60 dark:text-white/60 hover:text-[#F9423A] dark:hover:text-[#F9423A] transition-colors text-xs font-medium">{link}</Link>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <span className="text-xs font-bold text-brand-primary/50 dark:text-white/50">Bizi Takip Edin:</span>
             <div className="flex gap-4">
               {[Instagram, Twitter, Facebook, Youtube].map((Icon, i) => (
                 <a href="#" key={i} className="text-brand-primary/40 hover:text-[#F9423A] dark:text-white/40 dark:hover:text-[#F9423A] transition-colors">
                   <Icon size={18} />
                 </a>
               ))}
             </div>

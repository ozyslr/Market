            ...
            <div className="flex-1 overflow-y-auto px-6 py-8">
              {user ? (
                <div className="mb-8 p-4 bg-brand-secondary/20 dark:bg-zinc-900 rounded-2xl flex items-center gap-4 border border-brand-primary/5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-black uppercase text-brand-primary/40 dark:text-white/40 tracking-widest">
                      {t('nav.hello')},
                    </p>
              ...
              <div className="space-y-10">
                <div className="space-y-4">
                  <h4 className="text-[10px] font-black uppercase text-accent tracking-[0.3em]">
                  ...
                  >
                    {categories
                      ...
                      .map((cat) => {
                        const IconComponent =
                          CATEGORY_ICONS[cat.slug] || CATEGORY_ICONS[cat.id] || Grid3X3;
                        return (
                          <motion.div key={cat.id} variants={itemVariants}>
                            ...
                            >
                              <span className="flex items-center gap-3 min-w-0">
                                <IconComponent
                                  size={18}
                                  className="text-brand-primary/40 dark:text-white/40 group-hover:text-accent transition-colors shrink-0"
                                />
                                <span className="text-sm font-black dark:text-white truncate">
                                  {cat.name ||
                                    (t(`category.${cat.id}`) !== `category.${cat.id}`
                                      ? t(`category.${cat.id}`)
                                      : cat.slug || `Kat. ${cat.id.slice(-4)}`)}
                                </span>
                              </span>
                              <ChevronRight
                                size={16}
                                className={cn(
                                  'shrink-0 text-brand-primary/20 group-hover:text-accent transition-colors',
                                  rtl && 'rotate-180',
                                )}
                              />
                            </Link>
                ...
                <div className="space-y-4">
                  <h4 className="text-[10px] font-black uppercase text-brand-primary/30 dark:text-white/30 tracking-[0.3em]">
                    {t('nav.my_account')}
                  </h4>
            ...
            <div className="p-6 border-t border-brand-primary/5 space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                >
                  ...
                  <span className="text-[10px] font-black uppercase text-brand-primary/40 dark:text-white/40">
                    {t('nav.cart_short', 'Sepet')}
                  </span>
                  {cartBadge && (
                ...
                {availableLanguages &&
                  availableLanguages.map((l) => (
                    <button
                      ...
                      className={cn(
                        'flex-1 px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-colors border whitespace-nowrap min-w-[80px]',
                        lang === l.code
                          ? 'bg-brand-primary text-white border-transparent'
                          : 'bg-transparent text-brand-primary dark:text-white border-brand-primary/10',
                      )}
                    >

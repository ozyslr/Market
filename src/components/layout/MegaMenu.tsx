        ...
        >
          <div className="max-w-[1700px] mx-auto flex min-h-[550px]">
            ...
            <div className="w-[300px] bg-zinc-50 dark:bg-zinc-900 border-e border-brand-primary/5 py-6">
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-primary/30 mb-4 px-8 underline underline-offset-8">
                {t('nav.categories_nav')}
              </h3>
              {categories
                .filter((c) => !c.parentId)
                .map((cat) => {
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        {t(`category.${cat.id}`) !== `category.${cat.id}`
                          ? t(`category.${cat.id}`)
                          : cat.name || cat.slug || `Kategori ${cat.id.slice(-4)}`}
                      </div>
                      <ChevronRight
                        size={14}
                        className={cn(
            ...
            <div className="flex-1 p-12 bg-white dark:bg-zinc-950 overflow-y-auto no-scrollbar">
              {activeCategory ? (
                <div className="flex flex-col gap-12">
                  <div className="flex justify-between items-end border-b border-brand-primary/10 pb-4">
                    <h2 className="text-3xl font-display font-black uppercase italic tracking-tighter text-brand-primary dark:text-white">
                      {t(`category.${activeCategory}`) !== `category.${activeCategory}`
                      ...
                      .map((sub) => {
                        ...
                        return (
                          <div key={sub.id} className="space-y-3">
                            ...
                            >
                              {sub.name || sub.slug || `Alt Kategori ${sub.id.slice(-4)}`}
                            </Link>
                  ...
                  {categories.find((c) => c.id === activeCategory)?.image && (
                    <Link
                      to={`/category/${activeCategory}`}
                      onClick={onClose}
                      className="mt-8 rounded-3xl overflow-hidden relative group h-[200px] cursor-pointer block border border-brand-primary/10 shadow-sm"
                    >
                      <img
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent flex flex-col justify-end p-8">
                        <span className="text-accent text-[10px] font-black uppercase tracking-widest mb-1 shadow-sm">
                          {t('home.new_arrivals') || 'New Collection'}
                        </span>
                ...
                <div className="h-full flex flex-col items-center justify-center opacity-20">
                  ...
                  <p className="font-display font-black uppercase tracking-widest text-xl italic">
                    {t('mega.select_category')}
                  </p>
                </div>
              )}
            </div>
              ...
              {activeCategory && (
                <div className="space-y-4">
                  ...
                  <Link
                    to={`/category/${activeCategory}`}
                    onClick={onClose}
                    className="mt-4 block relative rounded-[2rem] overflow-hidden h-48 shadow-lg group cursor-pointer"
                  >
                    <img
                      src={
                        categories.find((c) => c.id === activeCategory)?.image ||
                      ...
                      alt={
                        categories.find((c) => c.id === activeCategory)?.name || 'Category Promo'
                      }
                      referrerPolicy="no-referrer"
                      loading="lazy"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-brand-primary/90 via-brand-primary/40 to-transparent p-6 flex flex-col justify-end text-white">
                      <span className="text-[9px] font-black uppercase tracking-[0.4em] text-accent mb-1">
                        {t('mega.regional_deal')}
                      </span>
                      <h5 className="text-xl font-display font-black leading-tight uppercase italic mb-2">
                        ESKİYİ YENİLE
                      </h5>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {categories
                          ...
                          ?.brands?.map((brand, bIdx) => (
                            <div
                              key={bIdx}
                              className="bg-white dark:bg-zinc-800 p-2 rounded-xl border border-brand-primary/5 hover:border-accent transition-colors flex items-center justify-center"
                            >
                              <span className="text-[10px] font-bold text-center text-brand-primary/80 dark:text-white/80 truncate w-full px-1">
                                {brand.name}
                              </span>
                            </div>
                          ))}

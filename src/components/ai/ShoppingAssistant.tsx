...
import {
  Sparkles, X, Send, Bot, Minimize2,
  Maximize2, Zap, BrainCircuit, Trash2, History,
} from 'lucide-react';
...
function saveHistory(msgs: { role: 'bot' | 'user'; text: string }[]) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(msgs.slice(-20))); } catch {}
}
...
export function ShoppingAssistant() {
  ...
  const [messages, setMessages] = useState<{ role: 'bot' | 'user'; text: string }[]>(() => {
    ...
    return saved.length > 0 ? saved : [
      { role: 'bot', text: 'Merhaba! Ben Benim Olan alışveriş asistanınızım. Sipariş, kargo, iade, ödeme veya ürünler hakkında sorularınızı yanıtlayabilirim. Size nasıl yardımcı olabilirim?' },
    ];
  });
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'bot', text: response || 'Üzgünüm, şu anda yanıt veremiyorum. Lütfen tekrar deneyin.' }]);
    } catch {
      setMessages(prev => [...prev, { role: 'bot', text: 'Geçici bir sorun oluştu. Lütfen tekrar deneyin veya destek ekibiyle iletişime geçin.' }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const clearChat = () => {
    setMessages([{ role: 'bot', text: 'Merhaba! Sohbet geçmişi temizlendi. Size nasıl yardımcı olabilirim?' }]);
          ...
          >
            ...
            <div className="p-6 bg-brand-primary text-white relative flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-4 relative z-10">
                ...
                <div>
                  <h3 className="text-sm font-black uppercase tracking-widest italic">Benim Olan Asistan</h3>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-ping" />
                    <span className="text-[10px] font-bold text-white/60 tracking-widest">7/24 AI Destek</span>
                  </div>
                </div>
              </div>
              <div className="flex gap-2 relative z-10">
                <button onClick={clearChat} className="p-2 hover:bg-white/10 rounded-lg transition-colors" title="Sohbeti Temizle"><Trash2 size={14} /></button>
              ...
              {messages.map((msg, i) => (
                <div key={i} className={cn('flex gap-3', msg.role === 'user' ? 'flex-row-reverse' : '')}>
                  <div className={cn('w-8 h-8 rounded-xl flex items-center justify-center shrink-0',
                    ...
                    {msg.role === 'bot' ? <Bot size={16} /> : <span className="text-[11px] font-black">S</span>}
                  </div>
                  <div className={cn('text-xs leading-relaxed rounded-2xl px-4 py-3 max-w-[75%]',
                    msg.role === 'bot'
                ...
                <div className="flex gap-3">
                  ...
                  <div className="bg-brand-secondary rounded-2xl px-4 py-3">
                    <div className="flex gap-1.5">
                      <div className="w-2 h-2 bg-brand-primary/30 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                      <div className="w-2 h-2 bg-brand-primary/30 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                      <div className="w-2 h-2 bg-brand-primary/30 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                    </div>
                  </div>
                </div>
              )}
            ...
            {messages.length <= 1 && (
              <div className="px-4 pb-2">
                <p className="text-[9px] font-black uppercase tracking-widest text-brand-primary/30 mb-2">Hızlı Sorular</p>
                <div className="flex flex-wrap gap-1.5">
                  {QUICK_REPLIES.map((qr, i) => (
                    <button key={i} onClick={() => sendMessage(qr.label)}

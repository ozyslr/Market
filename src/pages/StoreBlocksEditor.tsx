      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-brand-primary/40 mt-1">
            Sayfa bloklarını sırala, aktif/pasif yap
          </p>
        </div>
        <button
          onClick={handleSave}
          ...
          <div
            ...
            className={cn(
              ...
              block.enabled
                ...
                : 'bg-brand-secondary/30 border-brand-primary/5 opacity-50',
            )}
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-brand-primary/40 uppercase">
                {block.type.replace(/_/g, ' ')} · Sıra {block.order}
              </p>
            </div>
            <button
              onClick={() => toggleBlock(idx)}
              className="p-2 rounded-lg hover:bg-brand-secondary transition-all"
              title={block.enabled ? 'Pasif yap' : 'Aktif yap'}
            >
              {block.enabled ? (
                <Eye size={16} className="text-green-500" />
              ) : (
                <EyeOff size={16} className="text-brand-primary/20" />
              )}
            </button>
          </div>
        ))}

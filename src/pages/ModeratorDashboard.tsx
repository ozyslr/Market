...
import { ShoppingBag, RotateCcw, Star, MessageSquare, Shield, LogOut, Package, CheckCircle, XCircle, Edit3, Loader2, X } from 'lucide-react';
import { cn } from '@/lib/utils';
import { signOut } from 'firebase/auth';
import { auth } from '@/lib/firebase';
import { getPendingProducts, approveProduct, rejectProduct, requestChanges } from '@/services/moderationService';
...
function ModerationQueue() {
  ...
  useEffect(() => {
    getPendingProducts().then(p => { setProducts(p); setLoading(false); });
  }, []);
  ...
  if (loading) return <div className="flex justify-center py-16"><Loader2 className="w-8 h-8 animate-spin text-purple-600" /></div>;

  if (products.length === 0) return (
    <div className="flex flex-col items-center justify-center py-24 text-[#1A1033]/30">
      ...
      {selected ? (
        <div className="flex-1 bg-white rounded-4xl p-8 overflow-y-auto shadow-sm border border-[#F8F8FA]">
          <div className="flex gap-6 mb-6">
            <img src={selected.images[0]} alt="" className="w-40 h-40 rounded-2xl object-cover shrink-0" referrerPolicy="no-referrer" />
            <div>
              ...
              <p className="text-lg font-black text-[#1A1033]">{selected.price.toLocaleString('tr-TR')} ₺</p>
              <p className="text-[10px] text-[#1A1033]/40 mt-1">Stok: {selected.stock} · Kategori: {selected.categoryId}</p>
            </div>
          </div>

          <div className="mb-6">
            <p className="text-[10px] font-black uppercase tracking-widest text-[#1A1033]/40 mb-2">Açıklama</p>
            <p className="text-sm text-[#1A1033]/70 leading-relaxed">{selected.description}</p>
          </div>

          {selected.images.length > 1 && (
            <div className="flex gap-2 mb-6 flex-wrap">
              {selected.images.map((img, i) => (
                <img key={i} src={img} alt="" className="w-16 h-16 rounded-xl object-cover" referrerPolicy="no-referrer" />
              ))}
            </div>
          )}
          ...
          <div className="flex gap-3 pt-4 border-t border-[#F8F8FA]">
            ...
            >
              {actionLoading === 'approve' ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle size={14} />} Onayla
            </button>
            <button
              onClick={() => { setNoteModal({ type: 'changes' }); setNote(''); }}
              disabled={!!actionLoading}
            ...
            <button
              onClick={() => { setNoteModal({ type: 'reject' }); setNote(''); }}
              disabled={!!actionLoading}
        ...
        <div className="flex-1 flex items-center justify-center text-[#1A1033]/20">
      ...
      {noteModal && (
        <div className="fixed inset-0 z-200 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-4xl p-8 w-full max-w-md shadow-2xl">
            ...
            <p className="text-xs text-[#1A1033]/50 mb-5 line-clamp-1">{selected?.title}</p>
            ...
            <textarea
              ...
              rows={3}
              placeholder={noteModal.type === 'reject' ? 'Görseller yetersiz...' : 'Açıklama eksik, fiyat uygunsuz...'}
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setNoteModal(null)} className="flex-1 py-3 bg-[#F8F8FA] text-[#1A1033] rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#1A1033]/10 transition-all">İptal</button>
              <button
                ...
                className={cn(
                  ...
                  noteModal.type === 'reject' ? 'bg-red-500 hover:bg-red-600' : 'bg-yellow-500 hover:bg-yellow-600'
                )}
              >
                {actionLoading ? <Loader2 size={14} className="animate-spin" /> : noteModal.type === 'reject' ? <XCircle size={14} /> : <Edit3 size={14} />}
...
export function ModeratorDashboard() {
  ...
  const renderTab = () => {
    switch (activeTab) {
      case 'queue': return <ModerationQueue />;
    ...
    <div className="min-h-screen bg-[#F8F8FA] flex">
      ...
      <aside className="w-64 bg-white border-r border-[#F8F8FA] flex flex-col py-8 px-4 shrink-0 shadow-sm">
        <div className="flex items-center gap-2 mb-8 px-2">
          ...
          <div>
            <p className="font-black text-[#1A1033] text-sm uppercase tracking-tight">Moderatör</p>
            <p className="text-[10px] text-[#1A1033]/40 truncate max-w-[120px]">{user.name}</p>
          </div>
        </div>
            ...
            <button
              ...
              className={cn(
                ...
                activeTab === key
                  ...
                  : 'text-[#1A1033]/60 hover:bg-purple-50 hover:text-purple-600'
              )}
            >
              <Icon size={14} /> {label}
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-2 text-xs font-bold text-[#1A1033]/40 hover:text-red-500 transition-colors mt-4"
        >

    ...
    <div className="bg-white rounded-[3.5rem] p-8 lg:p-12 border border-[#F8F8FA] shadow-sm">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033]">Kampanyalar</h3>
        </div>
        ...
        <div className="mb-8 p-6 bg-[#F8F8FA] rounded-3xl border border-[#1A1033]/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-[#1A1033]/40 mb-1 block">Kampanya Adı *</label>
              <input value={form.name} onChange={e => set('name', e.target.value)} placeholder="Yaz sezonu indirimi..." className="w-full px-4 py-2.5 bg-white rounded-xl text-sm font-bold outline-none border border-transparent focus:border-accent/30" />
            </div>
            <div className="md:col-span-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-[#1A1033]/40 mb-1 block">Açıklama</label>
              <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={2} className="w-full px-4 py-2.5 bg-white rounded-xl text-sm font-bold outline-none border border-transparent focus:border-accent/30 resize-none" />
            </div>
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-[#1A1033]/40 mb-1 block">İndirim Tipi</label>
              <select value={form.discountType} onChange={e => set('discountType', e.target.value as CampaignDiscountType)} className="w-full px-4 py-2.5 bg-white rounded-xl text-sm font-bold outline-none border border-transparent focus:border-accent/30">
                <option value="percentage">Yüzde (%)</option>
                <option value="fixed">Sabit (₺)</option>
              </select>
            </div>
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-[#1A1033]/40 mb-1 block">İndirim Değeri</label>
              <input type="number" min={0} value={form.discountValue} onChange={e => set('discountValue', parseFloat(e.target.value) || 0)} className="w-full px-4 py-2.5 bg-white rounded-xl text-sm font-bold outline-none border border-transparent focus:border-accent/30" />
            </div>
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-[#1A1033]/40 mb-1 block">Hedef</label>
              <select value={form.targetType} onChange={e => set('targetType', e.target.value as CampaignTargetType)} className="w-full px-4 py-2.5 bg-white rounded-xl text-sm font-bold outline-none border border-transparent focus:border-accent/30">
            ...
            {form.targetType !== 'all_products' && (
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-[#1A1033]/40 mb-1 block">
                  {form.targetType === 'category' ? 'Kategori ID' : form.targetType === 'brand' ? 'Marka Adı' : 'Ürün ID\'leri (virgülle)'}
                </label>
                <input value={form.targetValue ?? ''} onChange={e => set('targetValue', e.target.value)} className="w-full px-4 py-2.5 bg-white rounded-xl text-sm font-bold outline-none border border-transparent focus:border-accent/30" />
              </div>
            )}
            <div>
            ...
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-[#1A1033]/40 mb-1 block">Bitiş</label>
              <input type="datetime-local" value={form.endDate} onChange={e => set('endDate', e.target.value)} className="w-full px-4 py-2.5 bg-white rounded-xl text-sm font-bold outline-none border border-transparent focus:border-accent/30" />
            </div>
            <div className="flex items-center gap-3">
              <button onClick={() => set('isActive', !form.isActive)} className="text-accent">
                {form.isActive ? <ToggleRight size={28} /> : <ToggleLeft size={28} className="text-[#1A1033]/30" />}
              </button>
              <span className="text-sm font-bold text-[#1A1033]/60">{form.isActive ? 'Aktif' : 'Pasif'}</span>
            </div>
          </div>
          <div className="flex items-center gap-3 mt-5">
            <button onClick={handleSave} disabled={saving || !form.name.trim()} className="px-5 py-2.5 bg-accent text-white rounded-2xl text-xs font-black uppercase disabled:opacity-50 hover:bg-accent/90 transition-colors">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {campaigns.map(c => {
              ...
              return (
                <tr key={c.id} className="border-b border-[#F8F8FA] hover:bg-[#F8F8FA]/50 transition-colors">
                  <td className="py-3">
                    <p className="font-bold text-[#1A1033] text-xs">{c.name}</p>
                    {c.description && <p className="text-[10px] text-[#1A1033]/40 line-clamp-1">{c.description}</p>}
                  </td>
                  ...
                  <td className="py-3 text-center text-[10px] font-bold text-[#1A1033]/60">
                    ...
                    {c.targetValue && <span className="block text-[#1A1033]/30">{c.targetValue}</span>}
                  </td>
                  ...
                  <td className="py-3 text-center">
                    <div className="flex items-center justify-center gap-1">
                      <button onClick={() => handleToggle(c)} className="p-1.5 rounded-lg hover:bg-[#F8F8FA] text-[#1A1033]/40 hover:text-accent transition-colors">
                        {c.isActive ? <ToggleRight size={16} className="text-accent" /> : <ToggleLeft size={16} />}
                      </button>
                      <button onClick={() => openEdit(c)} className="p-1.5 rounded-lg hover:bg-[#F8F8FA] text-[#1A1033]/40 hover:text-blue-500 transition-colors">
                        <Edit2 size={14} />
                      </button>
                      <button onClick={() => handleDelete(c.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-[#1A1033]/40 hover:text-red-500 transition-colors">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>

...
import { Plus, Trash2, ToggleLeft, ToggleRight, Loader2, Ticket, Clock, Tag } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#F8F8FA] p-6 lg:p-10">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-brand-primary/30">
            <Ticket size={18} />
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div
              key={i}
              className="bg-white rounded-2xl p-5 border border-brand-primary/5 flex items-center gap-4"
            >
              <div
                className={cn(
        ...
        <form
          ...
          className="bg-white rounded-[2rem] p-6 border border-brand-primary/5 space-y-4"
        >
        ...
        <div className="space-y-3">
          {coupons.length === 0 && (
            <div className="bg-white rounded-[2rem] p-16 text-center border border-dashed border-brand-primary/10">
              <Ticket size={40} className="mx-auto text-brand-primary/10 mb-4" />
              <p className="text-sm font-bold text-brand-primary/30">Henüz kupon oluşturmadınız</p>
              <p className="text-[10px] text-brand-primary/20 mt-1">
                Müşterilerinize özel indirim kodları tanımlayın
              </p>
            </div>
          )}
          {coupons.map((coupon) => (
            <div
              ...
              className={cn(
                ...
                coupon.isActive ? 'border-brand-primary/5' : 'border-brand-primary/5 opacity-50',
              )}
            >
              <div className="flex items-center gap-4">
                <span className="font-black text-brand-primary tracking-widest text-sm bg-[#F8F8FA] px-3 py-1.5 rounded-xl border border-brand-primary/10">
                  {coupon.code}
                </span>
                <div>

    ...
    <div className="bg-white rounded-[3.5rem] p-8 lg:p-12 border border-[#F8F8FA] shadow-sm space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] font-bold text-brand-primary/40 uppercase tracking-widest mt-1">
            Tüm admin işlemlerinin güvenlik günlüğü
          </p>
        </div>
        <button
          onClick={() => {
            const csv = [
              'tarih,aktör,aksiyon,varlık,detay',
              ...logs.map(
                (l) =>
          ...
          className="px-4 py-2.5 bg-[#F8F8FA] text-brand-primary rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2"
        >
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {entities.map((e) => {
          ...
          return (
            <button
              key={e}
              onClick={() => setFilter({ entityType: e, limit: 100 })}
              className={cn(
        ...
        <button
          ...
          className="ms-auto px-3 py-1.5 bg-[#F8F8FA] rounded-xl text-[9px] font-black uppercase tracking-widest text-brand-primary/40 flex items-center gap-1"
        >
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <Shield size={40} className="mx-auto text-brand-primary/10 mb-4" />
          <p className="text-sm font-bold text-brand-primary/30">Kayıt bulunamadı</p>
        </div>
      ) : (
          ...
          {logs.map((entry) => {
            ...
            return (
              ...
              >
                ...
                <ChevronDown
                  size={14}
                  className="text-brand-primary/20 group-hover:text-brand-primary/40 shrink-0"
                />
              </div>
            );
          })}
      ...
      {selectedLog && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-lg shadow-2xl space-y-4 max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-display font-black text-brand-primary">İşlem Detayı</h3>
              <button
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-brand-primary/40">Tarih</span>
                <span className="font-bold">
                  {new Date(selectedLog.createdAt).toLocaleString('tr-TR')}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-brand-primary/40">Aktör</span>
                <span className="font-bold">
                  {selectedLog.actorEmail} ({selectedLog.actorRole})
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-brand-primary/40">İşlem</span>
                <span className="font-bold">{AUDIT_ACTION_LABELS[selectedLog.action]}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-brand-primary/40">Varlık</span>
                <span className="font-bold">{selectedLog.entityLabel}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-brand-primary/40">Tip</span>
                <span className="font-bold">
              ...
              {selectedLog.details && (
                <div className="pt-2 border-t border-[#F8F8FA]">
                  <p className="text-brand-primary/40 text-xs mb-1">Detay</p>
                  <p className="text-sm">{selectedLog.details}</p>
                </div>
              )}
              {selectedLog.before && (
                <div className="pt-2 border-t border-[#F8F8FA]">
                  <p className="text-brand-primary/40 text-xs mb-1">Önceki Değer</p>
                  <pre className="text-[10px] bg-[#F8F8FA] rounded-xl p-3 overflow-x-auto">
              ...
              {selectedLog.after && (
                <div className="pt-2 border-t border-[#F8F8FA]">
                  <p className="text-brand-primary/40 text-xs mb-1">Yeni Değer</p>
                  <pre className="text-[10px] bg-[#F8F8FA] rounded-xl p-3 overflow-x-auto">

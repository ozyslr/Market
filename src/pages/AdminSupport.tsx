...
import { getAllTickets, addTicketMessage, updateTicketStatus, updateTicketPriority, SupportTicket, TicketStatus, TicketPriority } from '@/services/supportService';
import { cn } from '@/lib/utils';

const CATEGORY_LABELS: Record<string, string> = {
  order: 'Sipariş', payment: 'Ödeme', return: 'İade', account: 'Hesap', other: 'Diğer',
};
...
const PRIORITY_COLOR: Record<string, string> = {
  low: 'text-gray-400', medium: 'text-orange-400', high: 'text-red-500',
};
...
export function AdminSupport() {
  ...
  useEffect(() => {
    getAllTickets().then(data => { setTickets(data); setLoading(false); });
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-lg font-display font-black uppercase italic tracking-tight text-[#1A1033]">Destek Biletleri</h2>
        <div className="flex gap-2 flex-wrap">
          {(['open', 'in_progress', 'resolved', 'closed', 'all'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={cn('px-3 py-1.5 rounded-xl text-[10px] font-black transition-colors uppercase tracking-widest',
                filter === f ? 'bg-accent text-white' : 'bg-[#F8F8FA] text-[#1A1033]/60 hover:bg-accent/10')}>
              {f === 'all' ? `Tümü (${tickets.length})`
                : f === 'open' ? `Açık (${tickets.filter(t => t.status === 'open').length})`
                : f === 'in_progress' ? 'İşlemde'
                : f === 'resolved' ? 'Çözüldü'
                : 'Kapatıldı'}
            </button>
          ))}
            ...
            return (
              <button key={ticket.id} onClick={() => setSelected(ticket)}
                ...
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  ...
                  <span className={cn('px-2 py-0.5 rounded-lg text-[9px] font-black uppercase shrink-0', sc.color)}>{sc.label}</span>
                </div>
              ...
              <div className="px-6 py-4 border-b border-[#1A1033]/5">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    ...
                    <h3 className="text-sm font-black text-[#1A1033]">{selected.subject}</h3>
                  </div>
                  <div className="flex items-center gap-2 flex-wrap shrink-0">
                    <select value={selected.priority}
                      onChange={e => handlePriorityChange(selected.id, e.target.value as TicketPriority)}
                      className="px-2 py-1.5 rounded-xl border border-[#1A1033]/10 text-[10px] font-black outline-none">
              ...
              {selected.status !== 'closed' && (
                <div className="px-6 py-4 border-t border-[#1A1033]/5 flex gap-3">
                  <textarea value={replyText} onChange={e => setReplyText(e.target.value)} rows={2}
                    placeholder="Admin yanıtı yazın..." className="flex-1 px-4 py-2.5 rounded-xl border border-[#1A1033]/10 text-sm font-bold outline-none focus:border-accent/50 bg-[#F8F8FA] resize-none" />

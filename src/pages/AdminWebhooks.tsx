...
import {
  Webhook, Plus, Trash2, ToggleLeft, ToggleRight, Loader2, ExternalLink,
  CheckCircle, XCircle, Clock, RefreshCw, Zap, Play, Eye,
} from 'lucide-react';
import { cn } from '@/lib/utils';
import {
  getWebhooks, createWebhook, updateWebhook, deleteWebhook,
  getDeliveryLogs, testWebhook,
  WEBHOOK_EVENT_LABELS, WebhookEventType,
  WebhookSubscription, WebhookDeliveryLog,
} from '@/services/webhookService';

const EVENT_GROUPS: { label: string; events: WebhookEventType[] }[] = [
  { label: 'Siparişler', events: ['order.created', 'order.shipped', 'order.delivered', 'order.cancelled', 'order.returned'] },
...
export function AdminWebhooks() {
  ...
  const handleCreate = async () => {
    ...
    setForm({ ownerId: 'admin', name: '', url: '', events: [], secret: '', isActive: true, headers: {}, maxRetries: 3 });
    setShowForm(false);
    setSaving(false);
  };
  ...
  return (
    <div className="bg-white rounded-[3.5rem] p-8 lg:p-12 border border-[#F8F8FA] shadow-sm space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033]">Webhook Yönetimi</h3>
          <p className="text-[10px] font-bold text-[#1A1033]/40 uppercase tracking-widest mt-1">15 event tipi · Otomatik retry · HMAC imza</p>
        </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <label className="text-[9px] font-bold uppercase text-[#1A1033]/40 block mb-1">İsim</label>
                <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  placeholder="Sipariş Webhook'u" className="w-full px-3 py-2.5 bg-[#F8F8FA] rounded-xl text-sm font-bold outline-none" />
              </div>
              <div className="col-span-2">
                <label className="text-[9px] font-bold uppercase text-[#1A1033]/40 block mb-1">URL</label>
                <input value={form.url} onChange={e => setForm(f => ({ ...f, url: e.target.value }))}
                  placeholder="https://api.example.com/webhook" className="w-full px-3 py-2.5 bg-[#F8F8FA] rounded-xl text-sm font-mono font-bold outline-none" />
              </div>
              <div>
                <label className="text-[9px] font-bold uppercase text-[#1A1033]/40 block mb-1">Secret (opsiyonel)</label>
                <input value={form.secret} onChange={e => setForm(f => ({ ...f, secret: e.target.value }))}
                  placeholder="HMAC imza anahtarı" className="w-full px-3 py-2.5 bg-[#F8F8FA] rounded-xl text-sm font-bold outline-none" />
              </div>
              <div>
                <label className="text-[9px] font-bold uppercase text-[#1A1033]/40 block mb-1">Max Retry</label>
                <select value={form.maxRetries} onChange={e => setForm(f => ({ ...f, maxRetries: parseInt(e.target.value) }))}
                  className="w-full px-3 py-2.5 bg-[#F8F8FA] rounded-xl text-sm font-bold outline-none">
                  {[0, 1, 2, 3, 5].map(n => <option key={n} value={n}>{n} deneme</option>)}
                </select>
              </div>
            </div>
            ...
            {/* Event Selection */}
            <div>
              <label className="text-[9px] font-bold uppercase text-[#1A1033]/40 block mb-2">Event'ler ({form.events.length} seçili)</label>
              <div className="grid grid-cols-2 gap-3">
                {EVENT_GROUPS.map(group => (
                  <div key={group.label} className="bg-[#F8F8FA] rounded-xl p-3">
                    <p className="text-[9px] font-black uppercase text-[#1A1033]/40 mb-2">{group.label}</p>
                    <div className="space-y-1">
                      {group.events.map(event => (
                        <label key={event} className="flex items-center gap-2 cursor-pointer">
                          ...
                          <span className="text-[10px] font-bold text-[#1A1033]">{WEBHOOK_EVENT_LABELS[event]}</span>
                        </label>
                      ))}
                    </div>
                  </div>
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setShowForm(false)}
                className="flex-1 py-3 bg-[#F8F8FA] text-[#1A1033] rounded-xl text-[10px] font-black uppercase">İptal</button>
        ...
        {webhooks.map(wh => (
          <div key={wh.id} className={cn(
            ...
            <div className="flex items-start justify-between mb-3">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className={cn(
                    ...
                    wh.lastStatus === 'success' ? "bg-green-100 text-green-600" :
                    wh.lastStatus === 'failed' ? "bg-red-100 text-red-500" : "bg-zinc-100 text-zinc-500"
                  )}>
                    {wh.lastStatus === 'success' ? <CheckCircle size={8} /> :
                     wh.lastStatus === 'failed' ? <XCircle size={8} /> : <Clock size={8} />}
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => handleTest(wh)} disabled={testingId === wh.id}
                  className="p-2 text-[#1A1033]/30 hover:text-accent transition-colors" title="Test">
                  {testingId === wh.id ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />}
                </button>
                <button onClick={() => handleViewLogs(wh.id)}
                  className="p-2 text-[#1A1033]/30 hover:text-blue-500 transition-colors" title="Loglar">
                  <Eye size={14} />
                </button>
                <button onClick={() => handleToggle(wh)} className="transition-colors">
                  {wh.isActive ? <ToggleRight size={20} className="text-accent" /> : <ToggleLeft size={20} className="text-[#1A1033]/30" />}
                </button>
            ...
            <div className="flex flex-wrap gap-1">
              {wh.events.map(e => (
                <span key={e} className="px-2 py-0.5 bg-white rounded-lg text-[8px] font-black uppercase text-[#1A1033]/50">
                  {WEBHOOK_EVENT_LABELS[e]}
                </span>
              ))}
            </div>
            {wh.lastDeliveryAt && (
              <p className="text-[9px] text-[#1A1033]/30 mt-2">Son: {new Date(wh.lastDeliveryAt).toLocaleString('tr-TR')}</p>
            )}
          </div>
        ))}
      </div>
      ...
      {testResult && (
        <div className={cn(
          ...
          <p className="text-xs font-black">
            {testResult.success ? 'Başarılı!' : 'Başarısız'} — HTTP {testResult.status} ({testResult.duration}ms)
          </p>
          <button onClick={() => setTestResult(null)} className="text-[10px] underline mt-1">Kapat</button>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-3xl max-h-[85vh] overflow-y-auto shadow-2xl space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => { setViewingLogs(null); setLogs([]); }}
                className="p-2 hover:bg-[#F8F8FA] rounded-xl">✕</button>
            </div>
            {logsLoading ? (
              ...
              <div className="space-y-2">
                {logs.map(log => (
                  <div key={log.id} className={cn(
                    ...
                    log.status === 'success' ? "bg-green-50" : log.status === 'retrying' ? "bg-amber-50" : "bg-red-50"
                  )}>
                    <div className="flex items-center justify-between">
                      <span className="font-black">{WEBHOOK_EVENT_LABELS[log.eventType] || log.eventType}</span>
                      <span className={cn(
                        "px-2 py-0.5 rounded-lg text-[9px] font-black uppercase",
                        log.status === 'success' ? "bg-green-200 text-green-700" :
                        log.status === 'retrying' ? "bg-amber-200 text-amber-700" : "bg-red-200 text-red-700"
                      )}>{log.status}</span>
                    </div>
                    ...
                    <p className="text-[#1A1033]/30 mt-0.5">{new Date(log.createdAt).toLocaleString('tr-TR')}</p>
                  </div>
                ))}
              </div>
            )}

...
export function AdminProducts() {
  ...
  const toggleFlag = async (product: Product, flag: keyof Pick<Product, 'featured' | 'bestSeller' | 'isFlashDeal' | 'newArrival'>) => {
  ...
  const handleApprove = async (product: Product) => {
    ...
    try {
      ...
      setProducts(prev => prev.map(p => p.id === product.id ? { ...p, status: 'approved', moderationNote: '' } : p));
    } finally {
  ...
  const handleRejectConfirm = async () => {
    ...
    try {
      ...
      setProducts(prev => prev.map(p => p.id === rejectModal.product.id ? { ...p, status: 'rejected', moderationNote: rejectNote } : p));
      setRejectModal(null);
      setRejectNote('');
    } finally {
      setActionLoading(null);
  ...
  const bulkSetFlag = async (flag: keyof Pick<Product, 'featured' | 'bestSeller' | 'isFlashDeal' | 'newArrival'>, value: boolean) => {
    const ids = [...selected];
  ...
  const bulkApprove = async () => {
    const ids = [...selected].filter(id => (products.find(p => p.id === id)?.status ?? 'approved') === 'pending');
  ...
  return (
    <div className="bg-white rounded-[3.5rem] p-8 lg:p-12 border border-[#F8F8FA] shadow-sm">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h3 className="text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033]">Ürün Kataloğu</h3>
          {pendingCount > 0 && (
            <p className="text-xs font-bold text-yellow-600 mt-0.5">{pendingCount} ürün onay bekliyor</p>
          )}
        </div>
        ...
        {STATUS_FILTERS.map(f => (
          ...
          >
            ...
            {f.value === 'pending' && pendingCount > 0 && (
              <span className="ms-1.5 bg-yellow-400 text-[#1A1033] rounded-full px-1.5">{pendingCount}</span>
            )}
          </button>
        ))}
        {aiScanning && (
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-3 mb-4 p-3 bg-accent/5 rounded-2xl flex-wrap">
          ...
          <button onClick={bulkApprove} className="px-3 py-1 bg-green-100 text-green-700 rounded-lg text-[10px] font-black uppercase flex items-center gap-1 hover:bg-green-200 transition-all">
            <CheckCircle size={10} /> Toplu Onayla
          </button>
          {FLAGS.map(f => (
            <React.Fragment key={f.key}>
              <button onClick={() => bulkSetFlag(f.key, true)} className="px-3 py-1 bg-white rounded-lg text-[10px] font-black uppercase text-[#1A1033]/60 hover:text-accent border border-[#1A1033]/10 flex items-center gap-1">
                <f.icon size={10} className={f.color} /> {f.label} Ekle
              </button>
              <button onClick={() => bulkSetFlag(f.key, false)} className="px-3 py-1 bg-white rounded-lg text-[10px] font-black uppercase text-[#1A1033]/60 hover:text-red-500 border border-[#1A1033]/10 flex items-center gap-1">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#F8F8FA]">
              ...
              <th className="pb-3 text-start text-[10px] font-black uppercase tracking-widest text-[#1A1033]/30">Ürün</th>
              <th className="pb-3 text-start text-[10px] font-black uppercase tracking-widest text-[#1A1033]/30">Durum</th>
              <th className="pb-3 text-end text-[10px] font-black uppercase tracking-widest text-[#1A1033]/30">Fiyat</th>
              <th className="pb-3 text-end text-[10px] font-black uppercase tracking-widest text-[#1A1033]/30">Stok</th>
              {FLAGS.map(f => (
                <th key={f.key} className="pb-3 text-center text-[10px] font-black uppercase tracking-widest text-[#1A1033]/30">
          ...
          <tbody>
            {filtered.map(product => {
              ...
              return (
                <tr key={product.id} className={cn('border-b border-[#F8F8FA] hover:bg-[#F8F8FA]/50 transition-colors', selected.has(product.id) && 'bg-accent/5')}>
                  <td className="py-3">
                    <button onClick={() => toggleSelect(product.id)} className="text-[#1A1033]/30 hover:text-accent">
                      {selected.has(product.id) ? <CheckSquare size={16} className="text-accent" /> : <Square size={16} />}
                    </button>
                  </td>
                  <td className="py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {status === 'rejected' && product.moderationNote && (
                          <p className="text-[10px] text-red-500 italic mt-0.5 max-w-[180px] truncate" title={product.moderationNote}>{product.moderationNote}</p>
                        )}
                      </div>
                    </div>
                  </td>
                  <td className="py-3">
                    ...
                    {aiResults.has(product.id) && (
                      <span className={cn('ms-1.5 px-2 py-1 rounded-lg text-[8px] font-black uppercase',
                  ...
                  <td className="py-3 text-end font-bold text-[#1A1033] text-xs">{product.price.toLocaleString('tr-TR')} ₺</td>
                  <td className="py-3 text-end text-[10px] font-bold text-[#1A1033]/60">{product.stock}</td>
                  {FLAGS.map(f => {
                    const key = `${product.id}_${f.key}`;
                    const active = !!product[f.key];
                    return (
                      <td key={f.key} className="py-3 text-center">
                        <button onClick={() => toggleFlag(product, f.key)} disabled={toggling === key}
                          className={cn('p-1.5 rounded-lg transition-all', active ? 'bg-yellow-50' : 'bg-transparent hover:bg-[#F8F8FA]', toggling === key && 'opacity-50')}>
                          <f.icon size={14} className={cn(active ? f.color : 'text-[#1A1033]/20')} />
                        </button>
                      </td>
                    );
                  })}
                      ...
                      {status === 'pending' && (
                        <>
                          <button
                            ...
                            title="Onayla"
                          >
                            {actionLoading === product.id + '_approve' ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle size={14} />}
                          </button>
                          <button
                            onClick={() => { setRejectModal({ product }); setRejectNote(''); }}
        ...
        {filtered.length === 0 && <p className="text-center py-8 text-[#1A1033]/30 text-sm font-bold">Ürün bulunamadı</p>}
      </div>
      ...
      {rejectModal && (
        <div className="fixed inset-0 z-200 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-4xl p-8 w-full max-w-md shadow-2xl">
            <button onClick={() => setRejectModal(null)} className="absolute top-5 end-5 text-[#1A1033]/30 hover:text-[#1A1033]"><X size={18} /></button>
            <h4 className="text-lg font-display font-black uppercase italic mb-1">Ürünü Reddet</h4>
            <p className="text-xs text-[#1A1033]/50 mb-5 line-clamp-2">{rejectModal.product.title}</p>
            <label className="block text-[10px] font-black uppercase tracking-widest text-[#1A1033]/60 mb-2">Red Sebebi (Satıcıya bildirilir)</label>
            <textarea
              value={rejectNote}
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setRejectModal(null)} className="flex-1 py-3 bg-[#F8F8FA] text-[#1A1033] rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#1A1033]/10 transition-all">İptal</button>
              <button
              ...
              >
                {actionLoading ? <Loader2 size={14} className="animate-spin" /> : <XCircle size={14} />} Reddet
              </button>
            </div>
          </div>
        </div>

  ...
  return (
    <div className="bg-white rounded-[3.5rem] p-8 lg:p-12 border border-[#F8F8FA] shadow-sm space-y-8">
      <div className="flex items-center justify-between">
        <h3 className="text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033]">Finans Takibi</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {[
          { label: 'Brüt Satış Hacmi (GMV)', value: `${gmv.toLocaleString('tr-TR')} ₺`, icon: TrendingUp, color: 'text-blue-500', bg: 'bg-blue-50' },
          { label: 'Komisyon Geliri (%10)', value: `${commission.toLocaleString('tr-TR')} ₺`, icon: Percent, color: 'text-accent', bg: 'bg-accent/10' },
          { label: 'Satıcı Ödemesi', value: `${sellerPayouts.toLocaleString('tr-TR')} ₺`, icon: DollarSign, color: 'text-green-500', bg: 'bg-green-50' },
      ...
      {bySeller.length > 0 && (
        <div>
          <h4 className="text-sm font-black uppercase text-[#1A1033]/40 tracking-widest mb-4">Satıcı Bazında Dökümü (İlk 10)</h4>
      ...
      {delivered.length === 0 && <p className="text-center py-8 text-[#1A1033]/30 font-bold">Bu dönem için teslim edilmiş sipariş yok</p>}
    </div>
  );
}

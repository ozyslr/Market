...
export function AdminReports() {
  ...
  const KPIS = [
    ...
    },
    {
      ...
      value: `${avg.toLocaleString('tr-TR', { maximumFractionDigits: 0 })} ₺`,
      icon: TrendingUp,
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={exportCSV}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-primary text-white rounded-xl text-xs font-black hover:bg-accent transition-colors"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {KPIS.map(({ label, value, icon: Icon, color }) => (
          <div key={label} className="bg-white rounded-2xl p-4 border border-[#F8F8FA] shadow-sm">
            ...
            <p className="text-[10px] font-black uppercase text-brand-primary/40 mb-1">{label}</p>
            <p className="text-lg font-black text-brand-primary">{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-3xl border border-[#F8F8FA] shadow-sm overflow-hidden">
        <div className="p-4 border-b border-[#F8F8FA]">
          <p className="text-xs font-black uppercase text-brand-primary/40">
            Son Siparişler ({filtered.length} toplam)
          </p>
        </div>
        {filtered.length === 0 ? (
          <div className="py-16 text-center text-brand-primary/30 font-bold text-sm">
              ...
              <thead>
                <tr className="border-b border-[#F8F8FA]">
                  {['Sipariş ID', 'Tarih', 'Alıcı', 'Tutar', 'Durum'].map((h) => (
                    <th
                      key={h}
                      className="text-start p-3 font-black text-brand-primary/40 uppercase text-[10px]"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {filtered.slice(0, 20).map((o) => (
                  <tr key={o.id} className="border-b border-[#F8F8FA] hover:bg-[#F8F8FA]/50">
                    <td className="p-3 font-mono text-[10px] text-accent">
                      #{o.id.slice(-8).toUpperCase()}
                    </td>
                    <td className="p-3 text-brand-primary/60">
                      {new Date(o.createdAt).toLocaleDateString('tr-TR')}
                    </td>
                    <td className="p-3 text-brand-primary/60 max-w-[120px] truncate">
                      {o.buyerId}
                    </td>
                    <td className="p-3 font-bold text-brand-primary">
                      {(o.totalAmount || 0).toLocaleString('tr-TR')} ₺
                    </td>
                    <td className="p-3">
                      <span
                        className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${statusColor(o.status)}`}
                      >
                        {o.status}
                      </span>
                    </td>

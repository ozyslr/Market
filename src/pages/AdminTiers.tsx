...
import { getAllTierConfigs, updateTierConfig, getTierBenefits, SellerTier } from '@/services/sellerTierService';
...
export function AdminTiers() {
  ...
  useEffect(() => {
    getAllTierConfigs().then(data => { setTiers(data); setLoading(false); });
  }, []);
  ...
  return (
    <div className="bg-white rounded-[3.5rem] p-8 lg:p-12 border border-[#F8F8FA] shadow-sm space-y-8">
      <div>
        <h3 className="text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033]">Satıcı Kademeleri</h3>
        <p className="text-[10px] font-bold text-[#1A1033]/40 uppercase tracking-widest mt-1">Her kademe için ürün limiti, komisyon ve özellik yönetimi</p>
      </div>
          ...
          <div key={tier.tier} className={cn("rounded-2xl border-2 p-5", TIER_COLORS[tier.tier])}>
            <div className="flex items-center justify-between mb-3">
              <Star size={18} className={cn(
                ...
                tier.tier === 'silver' ? 'text-zinc-500' :
                tier.tier === 'bronze' ? 'text-orange-500' : 'text-zinc-400'
              )} />
            ...
            {editingTier === tier.tier ? (
              <div className="mt-3 space-y-2">
                <div>
                  <label className="text-[8px] font-bold uppercase text-[#1A1033]/40">Maks. Ürün</label>
                  <input type="number" value={editForm.maxProducts || ''}
                    onChange={e => setEditForm(f => ({ ...f, maxProducts: parseInt(e.target.value) || 0 }))}
                    className="w-full px-2 py-1.5 bg-white rounded-lg text-xs font-bold outline-none border" />
                </div>
                <div>
                  <label className="text-[8px] font-bold uppercase text-[#1A1033]/40">Komisyon %</label>
                  <input type="number" value={editForm.commissionRate || ''}
                    onChange={e => setEditForm(f => ({ ...f, commissionRate: parseFloat(e.target.value) || 0 }))}
                    className="w-full px-2 py-1.5 bg-white rounded-lg text-xs font-bold outline-none border" />
                </div>
                <div>
                  <label className="text-[8px] font-bold uppercase text-[#1A1033]/40">Aylık Ücret (₺)</label>
                  <input type="number" value={editForm.monthlyFee || ''}
                    onChange={e => setEditForm(f => ({ ...f, monthlyFee: parseFloat(e.target.value) || 0 }))}
                    className="w-full px-2 py-1.5 bg-white rounded-lg text-xs font-bold outline-none border" />
                </div>
              ...
              <>
                <div className="mt-3 space-y-1.5">
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="font-black text-[#1A1033]">{tier.maxProducts.toLocaleString()}</span>
                ...
                <div className="mt-3 pt-3 border-t border-[#1A1033]/10">
                  <p className="text-[8px] font-black uppercase tracking-widest text-[#1A1033]/30 mb-1.5">Özellikler</p>
                ...
                <div className="mt-2 flex gap-1.5">
                  {tier.apiAccess && <span className="px-1.5 py-0.5 bg-blue-100 text-blue-600 rounded text-[8px] font-black">API</span>}
                  {tier.couponCreation && <span className="px-1.5 py-0.5 bg-green-100 text-green-600 rounded text-[8px] font-black">Kupon</span>}
                  {tier.featuredPlacement && <span className="px-1.5 py-0.5 bg-purple-100 text-purple-600 rounded text-[8px] font-black">Öne Çıkan</span>}
                </div>

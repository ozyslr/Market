...
export function AdminAI() {
  ...
  const cards = [
    {
      ...
      desc: insights.topMarket
        ? `${insights.topMarket[0]} pazarı ${fmt(insights.topMarket[1])} ciroyla lider. Bu bölgeye reklam bütçesi artırılabilir.`
        : 'Yeterli sipariş verisi yok.',
      color: 'text-accent',
    },
    {
      icon: TrendingUp,
  ...
  return (
    <div className="space-y-10">
      <div className="bg-brand-primary rounded-[3.5rem] p-12 text-white relative overflow-hidden">
        ...
        <p className="text-white/50 text-sm font-medium italic max-w-2xl relative z-10">
          Mercora AI, gerçek satış verilerinizi analiz ederek aksiyona dönük öneriler üretir.
          Aşağıdaki içgörüler canlı sipariş ve katalog verisinden hesaplanmıştır.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {cards.map((c, i) => (
          <div
            key={i}
            className="bg-white rounded-[3rem] p-10 border border-[#F8F8FA] shadow-sm hover:shadow-xl transition-all group"
          >
            <div
              className={`w-14 h-14 rounded-2xl bg-[#F8F8FA] flex items-center justify-center mb-6 group-hover:scale-110 transition-transform ${c.color}`}
            >
              <c.icon size={26} strokeWidth={2.5} />
            </div>
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-primary/20 mb-2 italic">
              {c.label}
            </p>
            <h4 className="text-2xl font-display font-black uppercase tracking-tight text-brand-primary mb-4">
              {c.title}
            </h4>

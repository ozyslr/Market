...
import { 
  ...
  ArrowDownRight, CreditCard, Activity, Briefcase, MessageSquare,
  FileText, Percent, MousePointer2, Smartphone, Monitor, Laptop,
...
const SPARKLINE_DATA = [
  { v: 10 }, { v: 25 }, { v: 15 }, { v: 45 }, { v: 30 }, { v: 60 }, { v: 50 }, { v: 80 }
];

const RECENT_ORDERS = [
  { id: '#TRND-84562', customer: 'Mehmet Kaya', product: 'iPhone 15 Pro 256GB', amount: '84.999 ₺', status: 'Kargoda', date: '30 May 2026' },
  { id: '#TRND-84561', customer: 'Ayşe Demir', product: 'Nike Air Max 270', amount: '4.299 ₺', status: 'Teslim Edildi', date: '30 May 2026' },
  { id: '#TRND-84560', customer: 'Ali Yılmaz', product: 'Samsung 65" 4K TV', amount: '28.999 ₺', status: 'Hazırlanıyor', date: '30 May 2026' },
  { id: '#TRND-84559', customer: 'Zeynep Çelik', product: 'MacBook Air M2', amount: '38.999 ₺', status: 'Kargoda', date: '30 May 2026' },
];

const TOP_SELLING = [
  { id: 1, name: 'iPhone 15 Pro 256GB', sales: '2.345 sipariş', price: '84.999 ₺', image: 'https://images.unsplash.com/photo-1695048133142-1a20484d2569?w=100&h=100&fit=crop' },
  { id: 2, name: 'Apple AirPods Pro 2', sales: '1.987 sipariş', price: '8.999 ₺', image: 'https://images.unsplash.com/photo-1627933611562-bdad39077974?w=100&h=100&fit=crop' },
  { id: 3, name: 'Nike Air Max 270', sales: '1.765 sipariş', price: '4.299 ₺', image: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=100&h=100&fit=crop' },
];
  ...
  if (user.role !== 'admin' && user.email !== 'ozyslr@gmail.com') {
    return (
      <div className="min-h-screen bg-[#F8F8FA] flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="text-2xl font-black uppercase italic tracking-tighter text-[#1A1033]">Access Denied</h2>
    ...
    <div className="flex h-screen bg-[#F8F8FA] overflow-hidden text-[#1A1033] relative">
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 px-4 overflow-y-auto custom-scrollbar-sidebar space-y-1">
          <SidebarItem id="dashboard" label="Özet Tablo" icon={LayoutDashboard} active={activeTab === 'dashboard'} onClick={setActiveTab} />
          <SidebarItem id="users" label="Kullanıcılar" icon={Users} active={activeTab === 'users'} onClick={setActiveTab} />
          <SidebarItem id="sellers" label="Satıcı Ortakları" icon={Briefcase} active={activeTab === 'sellers'} onClick={setActiveTab} />
          <SidebarItem id="products" label="Ürün Kataloğu" icon={Package} active={activeTab === 'products'} onClick={setActiveTab} />
          <SidebarItem id="orders" label="Sipariş Merkezi" icon={ShoppingBag} active={activeTab === 'orders'} onClick={setActiveTab} />
      ...
      <main className="flex-1 h-full overflow-y-auto no-scrollbar pt-16 lg:pt-0 bg-zinc-50 dark:bg-zinc-950">
        ...
        <header className="sticky top-0 z-30 bg-white/90 dark:bg-zinc-950/90 backdrop-blur-xl px-4 lg:px-12 py-4 lg:py-6 flex items-center justify-between border-b border-brand-primary/5 dark:border-white/5 gap-4">
           <div className="flex items-center gap-4">
              <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-2 bg-white rounded-xl shadow-sm border border-brand-primary/5 text-brand-primary">
                <Menu size={20} />
              </button>
              <div className="hidden sm:block">
                <h1 className="text-xl lg:text-3xl font-display font-black uppercase italic tracking-tight text-brand-primary dark:text-white">Genel <span className="text-[#F9423A] underline underline-offset-4 lg:underline-offset-8 decoration-2 lg:decoration-4">Bakiş</span></h1>
                <p className="text-[8px] lg:text-[10px] font-black text-brand-primary/30 dark:text-white/30 uppercase tracking-[0.2em] lg:tracking-[0.4em] mt-1 lg:mt-2">Core Matrix Düğümü</p>
              </div>
           </div>
           ...
           <div className="flex items-center gap-2 lg:gap-8 border-l border-brand-primary/10 pl-4 lg:pl-8 shrink-0">
              ...
              <button className="flex items-center gap-2 px-3 py-2 lg:px-4 lg:py-2.5 bg-white dark:bg-zinc-900 border border-brand-primary/5 dark:border-white/5 rounded-xl lg:rounded-2xl text-[9px] lg:text-[10px] font-black uppercase tracking-widest shadow-sm text-brand-primary dark:text-white">
                 <Languages size={18} /> <span className="hidden xs:inline">TR</span> <ChevronRight size={14} className="opacity-30" />
              </button>
           </div>
        </header>
        ...
        <div className="p-4 md:p-8 lg:p-12 max-w-[1700px] mx-auto space-y-8 lg:space-y-12">
           ...
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10">
              ...
              <div className="lg:col-span-8 bg-white rounded-[2rem] lg:rounded-[3.5rem] p-6 lg:p-12 border border-[#F8F8FA] shadow-sm relative overflow-hidden group">
                 <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 lg:mb-12 gap-4">
                    <div>
                       <h3 className="text-xl lg:text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033]">Satış Performansı</h3>
                    </div>
                    <div className="flex flex-wrap items-center gap-4 lg:gap-8">
                       <div className="flex items-center gap-4 lg:gap-6">
                          <div className="flex items-center gap-2">
                             <div className="w-2.5 h-2.5 rounded-full bg-[#6D28D9]" />
                             <span className="text-[10px] font-black uppercase text-[#1A1033]/40 tracking-widest leading-none">Toplam Satış</span>
                          </div>
                          <div className="flex items-center gap-2">
                             <div className="w-2.5 h-2.5 rounded-full bg-[#10B981]" />
                             <span className="text-[10px] font-black uppercase text-[#1A1033]/40 tracking-widest leading-none">Komisyon Geliri</span>
                          </div>
                       </div>
                 ...
                 <div className="h-[400px] w-full mt-4">
                    <ResponsiveContainer width="100%" height="100%">
                       <AreaChart data={SALES_PERFORMANCE}>
                          ...
                          <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#1A1033', opacity: 0.3, fontWeight: 800 }} dy={10} />
                          <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#1A1033', opacity: 0.3, fontWeight: 800 }} tickFormatter={v => `₺${v/1000}k`} />
                          <Tooltip 
                            contentStyle={{ backgroundColor: '#1A1033', borderRadius: '20px', border: 'none', padding: '15px' }}
                            itemStyle={{ color: '#fff', fontSize: '11px', fontWeight: 'bold' }}
                          />
                          <Area type="monotone" dataKey="purple" stroke="#6D28D9" strokeWidth={5} fill="url(#purpleGradient)" animationDuration={2000} />
                          <Area type="monotone" dataKey="green" stroke="#10B981" strokeWidth={4} fill="url(#greenGradient)" animationDuration={2500} />
                       </AreaChart>
                    </ResponsiveContainer>
                 </div>
              </div>
              ...
              <div className="lg:col-span-4 bg-white rounded-[2rem] lg:rounded-[3.5rem] p-6 lg:p-12 border border-[#F8F8FA] shadow-sm flex flex-col">
                 <div className="flex items-center justify-between mb-2">
                    <h3 className="text-xl lg:text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033]">Sipariş Durumu</h3>
                 ...
                 <div className="flex-1 flex flex-col justify-center items-center py-6 lg:py-10 relative">
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                       <p className="text-3xl lg:text-5xl font-display font-black text-[#1A1033] tracking-tighter">18,392</p>
                       <p className="text-[8px] lg:text-[10px] font-black uppercase tracking-[0.2em] text-[#1A1033]/30 mt-1 lg:mt-2">Toplam</p>
                    </div>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              ...
              <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-5 gap-6">
                 {[
                   { label: 'Aktif Kullanıcılar', value: '7,846', sub: 'Şu anda aktif', icon: Users, color: '#6D28D9' },
                   { label: 'Aktif Satıcılar', value: '1,253', sub: 'Şu anda aktif', icon: ShoppingBasket, color: '#10B981' },
                   { label: 'Stokta Olan Ürünler', value: '32,457', sub: 'Toplam ürün', icon: Package, color: '#3B82F6' },
                   { label: 'Bekleyen Onaylar', value: '89', sub: 'Ürün & satıcı', icon: Clock, color: '#F59E0B' },
                   { label: 'Destek Talepleri', value: '23', sub: 'Açık talepler', icon: MessageCircle, color: '#EF4444' },
                 ].map((stat, i) => (
                   <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-[#F8F8FA] shadow-sm hover:shadow-xl transition-all group">
                      <div className="flex flex-col items-center text-center">
                         <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center mb-6 shadow-sm group-hover:scale-110 transition-transform", "bg-[#F8F8FA]")}>
                            <stat.icon size={22} className="text-[#1A1033]/20 group-hover:text-accent transition-colors" />
                         </div>
                         <p className="text-[9px] font-black uppercase tracking-widest text-[#1A1033]/30 mb-2 leading-tight">{stat.label}</p>
                         <h4 className="text-2xl font-display font-black text-[#1A1033] tracking-tighter mb-1">{stat.value}</h4>
                         <p className="text-[8px] font-bold text-[#1A1033]/20 uppercase tracking-widest">{stat.sub}</p>
              ...
              <div className="lg:col-span-4 bg-white rounded-[2rem] lg:rounded-[3.5rem] p-6 lg:p-12 border border-[#F8F8FA] shadow-sm flex flex-col h-full">
                 <div className="flex items-center justify-between mb-8 lg:mb-10">
                    <h3 className="text-xl lg:text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033]">Son Aktiviteler</h3>
                 ...
                 <div className="flex-1 space-y-8 lg:space-y-10">
                    {[
                      { icon: ShoppingBasket, color: 'bg-green-100 text-green-600', title: 'Yeni sipariş #MRC-84562', desc: '₺2,599.00 tutarında sipariş oluşturuldu.', time: '2 dakika önce' },
                      { icon: Users, color: 'bg-blue-100 text-blue-600', title: 'Yeni satıcı başvurusu', desc: 'TechStore başvurusu onay bekliyor.', time: '15 dakika önce' },
                      { icon: Package, color: 'bg-purple-100 text-purple-600', title: 'Ürün onaylandı', desc: 'Kablosuz Kulaklık Pro ürünü onaylandı.', time: '1 saat önce' },
                      { icon: RefreshCcw, color: 'bg-red-100 text-red-600', title: 'İade talebi', desc: '#MRC-84510 siparişi için iade talebi.', time: '2 saat önce' },
                    ].map((activity, i) => (
                      <div key={i} className="flex gap-6 group cursor-pointer transition-transform hover:translate-x-1">
           ...
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              ...
              <div className="lg:col-span-8 bg-white rounded-[2rem] lg:rounded-[4rem] border border-[#F8F8FA] shadow-sm overflow-hidden flex flex-col">
                 <div className="p-6 lg:p-12 lg:pb-8 border-b border-[#F8F8FA] flex items-center justify-between">
                    <h3 className="text-xl lg:text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033]">Son Siparişler</h3>
                    <button className="text-[10px] font-black uppercase text-accent hover:underline">Tümü →</button>
                 </div>
              ...
              <div className="lg:col-span-4 flex flex-col gap-10">
                 ...
                 <div className="bg-white rounded-[3.5rem] p-12 border border-[#F8F8FA] shadow-sm flex flex-col">
                    <div className="flex items-center justify-between mb-10">
                       <h3 className="text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033]">En Çok Satan Ürünler</h3>
                       <button className="text-[10px] font-black uppercase text-accent hover:underline">Tümü →</button>
                    </div>
                 ...
                 <div className="bg-white rounded-[3.5rem] p-12 border border-[#F8F8FA] shadow-sm flex-1">
                    <h3 className="text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033] mb-10">Hızlı İşlemler</h3>
                    <div className="grid grid-cols-2 gap-6">
                       {[
                         ...
                         { id: 'reports', icon: BarChart3, label: 'Rapor Oluştur', color: 'green-500' },
                         { id: 'seed', icon: Database, label: seeding ? 'Seeding...' : 'Seed Data', color: 'pink-500', onClick: handleSeed },
                       ].map((action: any, i) => (
                         <button 
                           key={i} 
                         ...
                         >
                            <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center bg-white shadow-sm group-hover:scale-110 transition-transform duration-500", `text-accent`)}>
                               <action.icon size={26} strokeWidth={2} />
                            </div>
                            <span className="text-[10px] font-black uppercase tracking-widest text-[#1A1033]/40 group-hover:text-white transition-colors">{action.label}</span>
                         </button>
                       ))}
                    </div>
                 </div>
           ...
           <AnimatePresence>
             {seedResult && (
               ...
               >
                 ...
                 <p className="text-[11px] font-black uppercase tracking-widest flex-1">{seedResult}</p>
                 <button onClick={() => setSeedResult(null)}><X size={16} /></button>
               </motion.div>
             )}
           </AnimatePresence>

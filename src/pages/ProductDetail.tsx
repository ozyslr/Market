      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-6">
        ...
        <div className="grid lg:grid-cols-[3fr_2fr] gap-10 mb-16">
          ...
          <div className="lg:sticky lg:top-4 h-fit space-y-4">
            {/* Product Info Card */}
            <div className="bg-white rounded-2xl border border-brand-primary/5 shadow-sm p-5 md:p-6 space-y-5">
              {/* Brand & Badges */}
              ...
              {/* Title */}
              <h1 className="text-xl md:text-2xl font-display font-black tracking-tight text-brand-primary leading-[1.2]">{product.title}</h1>
            ...
            {(campaigns.length > 0 || cartDiscount > 0 || activeCoupons.length > 0) && (
              <div className="bg-gradient-to-br from-orange-50 to-yellow-50 rounded-3xl border border-orange-100/50 p-6 md:p-8 space-y-4">
                {campaigns.length > 0 && (
                  <div className="space-y-2">
                    {campaigns.slice(0, 2).map(c => (
                      <div key={c.id} className="flex items-start gap-2 p-2 bg-white/60 rounded-xl">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[9px] text-orange-600 font-bold">
                            {c.discountType === 'percentage' ? `%${c.discountValue}` : `${c.discountValue} TL`} indirim
                          </p>
                        </div>
                      </div>
                    ))}
                ...
                {cartDiscount > 0 && (
                  <div className="p-2 bg-white/60 rounded-xl border border-green-100">
                    <p className="text-[9px] font-black text-green-700 uppercase">£{cartDiscount.toFixed(2)} tasarruf</p>
                  </div>
                )}
        ...
        {boughtTogether.length > 0 && (
          <div className="bg-white rounded-[2rem] border border-brand-primary/5 shadow-sm p-6 md:p-8 mb-12">

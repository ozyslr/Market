...
function HomepageSectionsEditor() {
  ...
  const addSlide = (sectionId: string) => {
    ...
    setSections((prev) =>
      prev.map((s) =>
        s.id === sectionId
          ? { ...s, config: { ...s.config, slides: [...(s.config.slides || []), slide] } }
          : s,
      ),
    );
  };
      ...
      prev.map((s) =>
        s.id === sectionId
          ? {
              ...
              config: {
                ...s.config,
                slides: (s.config.slides || []).filter((sl) => sl.id !== slideId),
              },
            }
          : s,
      ),
    ...
    <div className="space-y-4">
      {sections.map((section, idx) => (
        <div
          key={section.id}
          className={cn(
            'bg-[#F8F8FA] rounded-3xl p-6 border-2 transition-all',
            section.enabled ? 'border-accent/10' : 'border-transparent opacity-60',
          )}
        >
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateConfig(section.id, {
                      filter: e.target.value as HomepageSection['config']['filter'],
                    })
                  }
            ...
            <div className="flex gap-4 flex-wrap">
              <div>
                ...
                <input
                  type="datetime-local"
                  value={
                    section.config.flashDealEndTime
                      ? section.config.flashDealEndTime.slice(0, 16)
                      : ''
                  }
                  onChange={(e) =>
                    updateConfig(section.id, {
                      flashDealEndTime: e.target.value
            ...
            <div>
              ...
              <div className="space-y-3">
                {(section.config.slides || []).map((slide) => (
                  <div
                    key={slide.id}
                    className="bg-white rounded-2xl p-4 border border-brand-primary/5"
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                      {[
                        ...
                        <div key={key}>
                          ...
                          <input
                            value={val}
                            onChange={(e) =>
                              updateSlide(section.id, slide.id, { [key]: e.target.value })
                            }
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateSlide(section.id, slide.id, { category: e.target.value })
                          }
                          className="w-full px-3 py-2 bg-[#F8F8FA] rounded-lg text-xs font-bold outline-none"
                      ...
                      <div>
                        ...
                        <input
                          value={slide.color}
                          onChange={(e) =>
                            updateSlide(section.id, slide.id, { color: e.target.value })
                          }
                          className="w-full px-3 py-2 bg-[#F8F8FA] rounded-lg text-xs font-bold outline-none"
                          placeholder="accent"
                        />
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {slide.image && (
                        <img
                          src={slide.image}
                          className="h-8 w-14 object-cover rounded-lg"
...
export function AdminCMS() {
  ...
  const [newFilterAttr, setNewFilterAttr] = useState<Partial<FilterAttribute>>({
    key: '',
    label: '',
    type: 'checkbox',
    options: [],
    productField: 'attributes',
  });
  ...
  const removeL3Item = async (
    subId: string,
    query: string,
    currentItems: { name: string; query: string }[],
  ) => {
                              ...
                              {deletingId === cat.id ? (
                                <div className="flex items-center gap-1 bg-white rounded-lg shadow-md px-2 py-1">
                                  ...
                                  <button
                                    ...
                                    className="px-1.5 py-0.5 bg-red-500 text-white rounded text-[9px] font-black"
                                  >
                                    Evet
                                  </button>
                                  <button
                                    onClick={() => setDeletingId(null)}
                                    className="px-1.5 py-0.5 bg-[#F8F8FA] rounded text-[9px] font-black"
                                  >
                                    Hayır
                                  </button>
                                </div>
                                          ...
                                          <div className="flex flex-wrap gap-1 pt-2 mb-1">
                                            {l3Items.map((item) => (
                                              ...
                                              >
                                                ...
                                                <button
                                                  onClick={() =>
                                                    removeL3Item(sub.id, item.query, l3Items)
                                                  }
                                          ...
                                          {editingL3 === sub.id ? (
                                            <div className="flex gap-1 mt-1">
                                              ...
                                              <input
                                                ...
                                                onKeyDown={(e) =>
                                                  e.key === 'Enter' &&
                                                  confirmAddL3Item(sub.id, l3Items)
                                                }
                                              />
                                              <button
                                                onClick={() => confirmAddL3Item(sub.id, l3Items)}
                                                className="px-2 py-1 bg-accent text-white rounded-lg text-[9px] font-black"
                                              >
                                                +
                                              </button>
                                              <button
        ...
        {isFormOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-white rounded-[2.5rem] overflow-hidden shadow-2xl p-8 z-10"
            >
              <div className="flex items-center justify-between mb-8">
                <div>
              ...
              <form onSubmit={handleFormSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <select
                    name="parentId"
                    defaultValue={editingCategory?.parentId || parentPreset}
                    className="w-full px-4 py-3 bg-[#F8F8FA] rounded-xl text-sm font-bold border border-transparent focus:border-accent/20 outline-none"
                  >
                ...
                <div className="border-t border-[#1A1033]/5 pt-6 space-y-4">
                    ...
                    {filterAttributes.map((attr, idx) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setFilterAttributes((prev) => prev.filter((_, i) => i !== idx))
                          }
                  ...
                  <div className="bg-[#F8F8FA] rounded-xl p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input
                        type="text"
                        placeholder="Alan (key): brand"
                        value={newFilterAttr.key ?? ''}
                        onChange={(e) => setNewFilterAttr((p) => ({ ...p, key: e.target.value }))}
                        className="px-3 py-2 bg-white rounded-lg text-xs font-bold outline-none"
                      />
                      <input
                        type="text"
                        placeholder="Başlık: Marka"
                        value={newFilterAttr.label ?? ''}
                        onChange={(e) => setNewFilterAttr((p) => ({ ...p, label: e.target.value }))}
                        className="px-3 py-2 bg-white rounded-lg text-xs font-bold outline-none"
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <select
                        value={newFilterAttr.type ?? 'checkbox'}
                        onChange={(e) =>
                          setNewFilterAttr((p) => ({
                            ...p,
                            type: e.target.value as FilterAttributeType,
                          }))
                        }
                        className="px-3 py-2 bg-white rounded-lg text-xs font-bold outline-none"
                      >
                      ...
                      <select
                        value={newFilterAttr.productField ?? 'attributes'}
                        onChange={(e) =>
                          setNewFilterAttr((p) => ({
                            ...p,
                            productField: e.target.value as 'top-level' | 'attributes',
                          }))
                        }
                        className="px-3 py-2 bg-white rounded-lg text-xs font-bold outline-none"
                      >
                    ...
                    {(newFilterAttr.type === 'checkbox' || newFilterAttr.type === 'select') && (
                      <input
                        type="text"
                        placeholder="Seçenekler: Samsung, Apple, Xiaomi"
                        value={(newFilterAttr.options ?? []).map((o) => o.value).join(', ')}
                        onChange={(e) =>
                          setNewFilterAttr((p) => ({
                            ...p,
                        ...
                        className="w-full px-3 py-2 bg-white rounded-lg text-xs font-bold outline-none"
                      />
                    )}
                    <button
                      type="button"
                      onClick={() => {
                        if (!newFilterAttr.key || !newFilterAttr.label) return;
                        setFilterAttributes((prev) => [...prev, newFilterAttr as FilterAttribute]);
                        setNewFilterAttr({
                          key: '',
                          label: '',
                          type: 'checkbox',
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-accent text-white rounded-xl text-xs font-black hover:bg-accent/90 transition-all"
                    >

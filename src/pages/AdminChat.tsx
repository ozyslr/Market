...
export function AdminChat() {
  ...
  const statusIcon = (status: string) => {
    switch (status) {
      case 'waiting': return <Circle size={10} className="text-yellow-500" />;
      case 'active': return <Circle size={10} className="text-green-500" />;
      case 'resolved': return <CheckCircle2 size={10} className="text-blue-500" />;
      default: return <Circle size={10} className="text-gray-300" />;
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-display font-black uppercase italic tracking-tight text-[#1A1033]">
          Canlı Destek
        </h2>
        {waitingCount > 0 && (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-1 space-y-1 max-h-[700px] overflow-y-auto pr-1">
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-[#F8F8FA] py-12 text-center">
              <MessageCircle size={28} className="mx-auto mb-2 text-[#1A1033]/20" />
              <p className="text-sm font-bold text-[#1A1033]/30">Aktif sohbet yok</p>
            </div>
          ) : sessions.filter(s => s.status !== 'closed').length === 0 && (
            <div className="bg-white rounded-2xl border border-[#F8F8FA] py-8 text-center">
              <MessageCircle size={28} className="mx-auto mb-2 text-[#1A1033]/10" />
              <p className="text-sm font-bold text-[#1A1033]/20">Tüm sohbetler kapalı</p>
            </div>
          )}
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-1">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-[11px] font-black text-[#1A1033] truncate">
              ...
              {session.lastMessage && (
                <p className="text-[10px] text-[#1A1033]/40 truncate mt-1">{session.lastMessage}</p>
              )}
              <p className="text-[8px] text-[#1A1033]/20 mt-1">{formatTime(session.lastMessageTs)}</p>
            </button>
          ))}
        </div>
        ...
        <div className="xl:col-span-2">
          {selectedSession ? (
            <div className="bg-white rounded-2xl border border-[#F8F8FA] flex flex-col" style={{ minHeight: 500 }}>
              {/* Header */}
              <div className="px-6 py-4 border-b border-[#1A1033]/5 flex items-center justify-between">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-[#1A1033]/30 mb-0.5">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4" style={{ maxHeight: 400 }}>
                {messages.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-[#1A1033]/20">
                    <MessageCircle size={32} className="mb-2" />
                    <p className="text-xs font-bold">Henüz mesaj yok</p>
                  </div>
                ) : messages.map((msg, i) => (
                  <div key={msg.id || i} className={cn('flex', msg.role === 'admin' ? 'justify-end' : 'justify-start')}>
                    <div className={cn(
                      ...
                      <p className={cn('text-[9px] mt-1', msg.role === 'admin' ? 'text-white/60' : 'text-[#1A1033]/30')}>
                        {formatTime(msg.ts)}
                      </p>
                    </div>
                  </div>
              ...
              {selectedSession.status !== 'closed' && (
                <div className="px-6 py-4 border-t border-[#1A1033]/5 flex gap-3">
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 rounded-xl border border-[#1A1033]/10 text-sm font-bold outline-none focus:border-accent/50 bg-[#F8F8FA]"
                  />
                  <button
                    onClick={handleSend}
            ...
            <div className="bg-white rounded-2xl border border-[#F8F8FA] flex flex-col items-center justify-center py-24 gap-3 text-[#1A1033]/20">
      ...
      {sessions.filter(s => s.status === 'closed').length > 0 && (
        <details className="bg-white rounded-2xl border border-[#F8F8FA]">
          <summary className="px-6 py-4 text-[11px] font-black uppercase tracking-widest text-[#1A1033]/40 cursor-pointer">

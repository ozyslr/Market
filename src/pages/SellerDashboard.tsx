...
import { 
  ...
  Activity, CreditCard, MessageSquare, Briefcase, RefreshCcw, Loader2, Percent,
        ...
        <div className="p-4 md:p-8 lg:p-12 max-w-[1700px] mx-auto space-y-8 lg:space-y-12 pb-32">
           ...
           <AnimatePresence mode="wait">
             {activeTab === 'dashboard' ? (
                ...
                <motion.div key="finance" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                  <div className="flex items-center justify-between">
                    <h3 className="text-2xl font-display font-black uppercase italic tracking-tighter text-[#1A1033]">Finans Özeti</h3>
                  ...
                  {financeLoading ? (
                    ...
                    const cutoff = financePeriod === 'all' ? null : (() => { const d = new Date(); d.setDate(d.getDate() - parseInt(financePeriod)); return d; })();
                    ...
                    return (
                      <>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                          {[
                            { label: 'Brüt Kazanç', value: `${grossEarnings.toLocaleString('tr-TR')} ₺`, icon: TrendingUp, color: 'text-blue-500', bg: 'bg-blue-50' },
                            { label: 'Komisyon Kesintisi (%10)', value: `${commission.toLocaleString('tr-TR')} ₺`, icon: Percent, color: 'text-[#F9423A]', bg: 'bg-[#F9423A]/10' },
                            { label: 'Net Ödeme', value: `${netEarnings.toLocaleString('tr-TR')} ₺`, icon: DollarSign, color: 'text-green-500', bg: 'bg-green-50' },
                          ].map(({ label, value, icon: Icon, color, bg }) => (
                            <div key={label} className="bg-white rounded-[2.5rem] p-8 flex items-center gap-4 border border-[#F8F8FA] shadow-sm">
                              <div className={`w-12 h-12 ${bg} rounded-xl flex items-center justify-center`}><Icon size={20} className={color} /></div>
                              <div>
                                <p className="text-[10px] font-bold text-[#1A1033]/40 uppercase tracking-widest mb-0.5">{label}</p>
                        ...
                        <div className="bg-white rounded-[2.5rem] border border-[#F8F8FA] shadow-sm overflow-hidden">
                          <div className="p-8 border-b border-[#F8F8FA]">
                            <h4 className="text-sm font-black uppercase tracking-widest text-[#1A1033]/40">Sipariş Bazında Kazanç</h4>
                          </div>
                          {delivered.length === 0 ? (
                            <p className="text-center py-12 text-[#1A1033]/30 font-bold text-sm">Bu dönem teslim edilen sipariş yok</p>
                          ) : (
                            <div className="overflow-x-auto">

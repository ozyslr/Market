...
export function SellerPriceAnalysis() {
    ...
    getProducts({ sellerId: user.id, includeNonApproved: false }).then((products) => {
      const batch = analyzeProductBatch(
        products.map((p) => ({
          ...
          name: p.title,
          image: p.images?.[0] || '',
          category: (p as any).categoryId || 'Genel',
          price: p.price,
        })),
      );
  ...
  const filtered =
    selectedCategory === 'all' ? analyses : analyses.filter((a) => a.category === selectedCategory);
  ...
  if (!summary)
    return (
      <div className="text-center py-24">
        <p className="text-sm font-bold text-brand-primary/30">Analiz edilecek ürün yok</p>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            {
              label: 'Analiz Edilen',
              value: summary.analyzedProducts,
              icon: BarChart3,
              color: 'text-blue-500',
              bg: 'bg-blue-50',
            },
            ...
            <motion.div
              ...
              className="bg-white rounded-2xl p-5 border border-brand-primary/5"
            >
              <div
                className={cn(
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] text-brand-primary/50 mt-0.5">
                30 adet/ay satış varsayımı ile hesaplanmıştır
              </p>
            </div>
          </div>
        )}
            ...
            <button
              ...
              className={cn(
                ...
                selectedCategory === 'all'
                  ...
                  : 'bg-white text-brand-primary/40 border border-brand-primary/5',
              )}
            >
              Tümü
            </button>
        ...
        <div className="bg-white rounded-[2rem] border border-brand-primary/5 overflow-hidden">
                        ...
                        <div className="flex items-center gap-3">
                          {a.productImage && (
                            <img
                              src={a.productImage}
                              alt={a.productName}
                              className="w-10 h-10 rounded-xl object-contain bg-[#F8F8FA] p-1"
                            />
                          )}
                          <div>
                            <p className="text-xs font-bold text-brand-primary line-clamp-1 max-w-[200px]">
                              {a.productName}
                            </p>
                            <p className="text-[9px] text-brand-primary/40">{a.category}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="font-black text-brand-primary">
                          {a.yourPrice.toFixed(2)} ₺
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="font-bold text-brand-primary/60">
                          {a.marketAverage.toFixed(2)} ₺
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs font-bold text-green-600">
                          {a.marketLow.toFixed(2)} ₺
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs font-bold text-red-500">
                          {a.marketHigh.toFixed(2)} ₺
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span
                          className={cn(
                            'inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase',
                            pos.color,
                          )}
                        >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={cn(
                            ...
                            a.potentialImpact > 0
                              ? 'text-green-600'
                              : a.potentialImpact < -50
                                ? 'text-red-500'
                                : 'text-brand-primary/40',
                          )}
                        >
                          {a.potentialImpact > 0 ? '+' : ''}
                          {a.potentialImpact.toFixed(0)} ₺/ay
                        </span>
                      </td>
                    </tr>
                  );
        ...
        {filtered.length > 0 && (
          <div className="bg-white rounded-[2rem] border border-brand-primary/5 p-6">
            <h3 className="text-sm font-black uppercase tracking-widest text-brand-primary/60 mb-4">
              Rakip Platform Detayı
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {(['Trendyol', 'Hepsiburada', 'AmazonTR', 'N11'] as const).map((platform) => {
                const platformPrices = filtered.flatMap((a) =>
                  a.competitors.filter((c) => c.platform === platform),
                );
                const avgPrice =
                ...
                return (
                  <div key={platform} className="bg-[#F8F8FA] rounded-2xl p-4">
                    <p className="text-[10px] font-black text-brand-primary mb-2">{platform}</p>
                    <p className="text-lg font-display font-black text-brand-primary">
                      {avgPrice.toFixed(2)} ₺
                    </p>
                    <p className="text-[9px] text-brand-primary/40 mt-0.5">Ortalama fiyat</p>
                    <div className="mt-2 flex items-center gap-1">
                      <span
                        className={cn(
                          'text-[9px] font-bold',
                          cheaperCount > filtered.length / 2 ? 'text-red-500' : 'text-green-500',
                        )}
                      >

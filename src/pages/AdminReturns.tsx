    ...
    <div className="bg-white rounded-[3.5rem] p-8 lg:p-12 border border-[#F8F8FA] shadow-sm">
      ...
      {returns.length === 0 ? (
        ...
        <div className="space-y-4">
          {returns.map(order => {
            const st = STATUS_LABEL[order.status] || { label: order.status, cls: 'bg-zinc-100 text-zinc-600' };
            return (
              <div key={order.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 bg-[#F8F8FA] rounded-2xl">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1">
                    <span className="text-xs font-black text-[#1A1033]">#{order.id.slice(-8).toUpperCase()}</span>
                    <span className={cn('text-[9px] font-black uppercase px-2 py-0.5 rounded-full', st.cls)}>{st.label}</span>
                  </div>
                  <p className="text-[10px] text-[#1A1033]/40">{new Date(order.createdAt).toLocaleDateString('tr-TR')} · {order.items.length} ürün</p>
                  <p className="text-xs font-bold text-[#1A1033] mt-0.5">{order.totalAmount?.toLocaleString('tr-TR') || '—'} ₺</p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => handleAction(order.id, 'refunded')} disabled={updating === order.id || order.status === 'refunded'}
                  ...
                  <button onClick={() => handleAction(order.id, 'cancelled')} disabled={updating === order.id || order.status === 'cancelled'}

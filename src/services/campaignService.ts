...
export function calcCampaignDiscount(
  ...
  const best = applicable.reduce((max, c) => {
    const val =
      c.discountType === 'percentage' ? orderTotal * (c.discountValue / 100) : c.discountValue;
...
export function getCartCampaignGifts(
  cartCampaigns: CartCampaign[],
): NonNullable<CartCampaign['giftProduct']>[] {
  return cartCampaigns.filter((cc) => cc.giftProduct).map((cc) => cc.giftProduct!);
}

...
export function analyzeSellerBehavior(
  seller: {
  ...
  if (productCount === 0) {
    return {
      sellerId: '', accountAgeDays, productCount: 0, avgProductPrice: 0,
      priceVolatility: 0, listingFrequency: 0, discountFrequency: 0,
      imageReuseRate: 0, categoryDiversity: 0, rapidPriceChanges: 0, suspiciousScore: 0,
    };
  }
  ...
  // Discount frequency
  const deepDiscounts = products.filter((p) => p.oldPrice && (p.oldPrice - p.price) / p.oldPrice > 0.3);
  ...
  return {
    sellerId: '', accountAgeDays, productCount, avgProductPrice,
    priceVolatility, listingFrequency, discountFrequency,
    imageReuseRate, categoryDiversity, rapidPriceChanges,
...
export function getBehaviorFlags(profile: BehaviorProfile): string[] {
  const flags: string[] = [];
  if (profile.accountAgeDays < 7) flags.push(`Yeni hesap (${Math.round(profile.accountAgeDays)} gün)`);
  if (profile.listingFrequency > 20) flags.push(`Yüksek listeleme hızı (${profile.listingFrequency.toFixed(1)}/hafta)`);
  if (profile.discountFrequency > 0.5) flags.push(`Sık derin indirim (%${Math.round(profile.discountFrequency * 100)})`);
  if (profile.imageReuseRate > 0.3) flags.push(`Görsel tekrar kullanımı (%${Math.round(profile.imageReuseRate * 100)})`);
  if (profile.priceVolatility > 1.0) flags.push(`Yüksek fiyat oynaklığı`);
  if (profile.rapidPriceChanges > 2) flags.push(`Hızlı fiyat değişimi (${profile.rapidPriceChanges} kez)`);
  return flags;
}

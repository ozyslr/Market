...
export function movingAverageDeviation(
  ...
  if (window.length < 3) return { isAnomaly: false, score: 0, method: 'moving_avg', details: [], threshold: thresholdMultiplier };
  ...
  return {
    ...
    score: Math.round(score),
    method: 'moving_avg',
    details: deviation > stdDev * thresholdMultiplier
...
export function computeMlFraudScore(
  ...
  sellerStats: { avgPrice: number; priceStdDev: number; recentPrices: number[]; avgDiscount: number },
  ...
  if (priceZ.isAnomaly) { anomalies.push(priceZ); flags.push(`Anormal fiyat (Z=${priceZ.score})`); }

  // 2. Price IQR vs category
  const priceIqr = iqrOutlier(product.price, categoryStats.priceQ1, categoryStats.priceQ3);
  if (priceIqr.isAnomaly) { anomalies.push(priceIqr); flags.push(`Fiyat IQR dışı (skor=${priceIqr.score})`); }

  // 3. Discount anomaly vs seller history
  const discountMa = movingAverageDeviation(product.discountPercent, sellerStats.recentPrices.map(() => sellerStats.avgDiscount));
  if (discountMa.isAnomaly) { anomalies.push(discountMa); flags.push(`Anormal indirim (skor=${discountMa.score})`); }
  ...
  if (sellerPriceZ.isAnomaly) { anomalies.push(sellerPriceZ); flags.push(`Satıcı fiyat sapması (Z=${sellerPriceZ.score})`); }

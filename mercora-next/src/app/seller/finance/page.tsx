...
export default function SellerFinancePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F8FA] p-6 lg:p-10 text-gray-900">
          ...
          <>
            ...
            <div className="bg-white rounded-2xl border border-[#1A1033]/5 overflow-hidden">
              <div className="px-6 py-4 border-b border-[#1A1033]/5 flex items-center justify-between">
                ...
                <Link href="/seller/orders" className="text-[10px] font-black text-accent hover:underline flex items-center gap-1">
                      ...
                      {recentOrders.map(order => {
                        ...
                        return (
                          <tr key={order.id} className="border-b border-[#1A1033]/5 hover:bg-[#F8F8FA] transition-colors">
                            <td className="px-5 py-3">
                              <Link href={`/orders/${order.id}`} className="text-[11px] font-black text-accent hover:underline font-mono">
                                #{order.id.slice(0, 8)}
                              </Link>
                            </td>
                            <td className="px-5 py-3 text-[11px] text-[#1A1033]/60">
      ...
      {showPayoutModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl space-y-5">
            ...
            <p className="text-[10px] font-bold text-[#1A1033]/40 uppercase tracking-widest text-zinc-500">
            ...
            <div>
              ...
              <input type="number" min="0" max={availableBalance}
                ...
                className="w-full px-4 py-3 rounded-xl border border-[#1A1033]/10 text-sm font-bold outline-none focus:ring-2 focus:ring-accent bg-white text-gray-900" />
            </div>
            <div>
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setShowPayoutModal(false)}
                className="flex-1 py-3 border border-[#1A1033]/10 text-[#1A1033] font-black text-xs rounded-xl hover:bg-[#F8F8FA] transition-all uppercase tracking-widest cursor-pointer">
                İptal
              </button>
              <button
                onClick={async () => {
                ...
                className="flex-1 py-3 bg-accent text-white font-black text-xs rounded-xl hover:opacity-90 transition-all disabled:opacity-50 uppercase tracking-widest flex items-center justify-center gap-2 cursor-pointer"
              >

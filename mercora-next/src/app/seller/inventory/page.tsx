        ...
        complete: (result) => {
          ...
          const rows: Partial<Product>[] = (result.data as Record<string, string>[]).map((raw, i) => {
            ...
            return {
              ...
              visibility:            (['public','hidden','draft'].includes(raw.visibility) ? raw.visibility : 'public') as Product['visibility'],
            };
          });
          resolve({ rows, errors });
        },
  ...
  async function confirmBulkUpload() {
    ...
    for (let i = 0; i < bulkPreview.length; i += BATCH) {
      ...
      await Promise.all(chunk.map(p => {
        ...
        return createProduct({ ...p, sellerId: user.id, slug, rating: 0, reviewsCount: 0, status: 'pending' } as Omit<Product, 'id'>);
      }));
      setBulkProgress(Math.min(i + BATCH, bulkPreview.length));
    }
    const refreshed = await getProducts({ sellerId: user.id, includeNonApproved: true });
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i += BATCH) {
        ...
        const created = await Promise.all(chunk.map(async p => {
          ...
          const id = await createProduct({ ...p, sellerId: user?.id, slug, hsCode: '8518.21.00', originCountry: 'Turkey' } as Omit<Product, 'id'>);
          return { id, ...p, sellerId: user?.id, slug } as Product;
        }));
      ...
      <div className="w-20 bg-brand-primary flex flex-col items-center py-8 gap-8 hidden lg:flex">
         ...
         <nav className="flex flex-col gap-6">
           {[
             ...
             <button
               key={item.id}
               onClick={() => setActiveTab(item.id as 'inventory' | 'bulk' | 'analytics')}
               className={cn(
                           ...
                           >
                             ...
                             <td className="px-8 py-6">
                               <div className="flex items-center gap-2">
                                 ...
                                 {product.status === 'rejected' && (product as any).moderationNote && (
                                   <span className="text-[9px] text-red-400 italic truncate max-w-[120px]" title={(product as any).moderationNote}>{(product as any).moderationNote}</span>
                                 )}
                               </div>
                             </td>
                             <td className="px-8 py-6">
                    ...
                    <div className="space-y-4">
                       ...
                       <div className="space-y-3">
                          <div className="flex justify-between items-center bg-green-50 p-3 rounded-xl border border-green-100">
                             ...
                             <span className="text-xs font-mono font-bold text-green-700">{(viewingProduct as any).hsCode || '8518.21.00'}</span>
                          </div>
        ...
        {isFormOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.9, opacity: 0, y: 20 }}
              className="relative w-full max-w-2xl bg-white rounded-[2rem] p-8 shadow-2xl overflow-y-auto max-h-[85vh]"
            >
              <button
                onClick={() => { setIsFormOpen(false); setEditingProduct(null); }}
                className="absolute top-6 right-6 w-10 h-10 bg-brand-secondary rounded-full flex items-center justify-center text-brand-primary hover:bg-accent hover:text-white transition-all z-20"
              ...
              <h2 className="text-2xl font-display font-black uppercase italic text-brand-primary mb-6">
                {editingProduct ? 'Update Artifact' : 'Create New Artifact'}
              </h2>
              <ProductForm

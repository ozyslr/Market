      ...
      .then(snap => {
        if (snap.exists()) {
          ...
          setForm(prev => ({
            ...
            estimatedDeliveryDays: (data as any).estimatedDeliveryDays ?? 3,
          }));
        } else {
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {TABS.map(({ key, label, icon: Icon }) => (
          <button
            ...
            className={cn(
              'flex items-center gap-2 px-4 py-2 rounded-2xl text-xs font-black transition-colors cursor-pointer',
              activeTab === key
      ...
      <div className="bg-white rounded-[3rem] p-8 border border-[#F8F8FA] shadow-sm space-y-5">
          ...
          <>
            ...
            <div>
              ...
              <input
                ...
                className="w-24 border border-[#F8F8FA] rounded-xl p-3 text-sm focus:outline-none focus:border-accent bg-white text-gray-900"
              />
            </div>
            <Field
              label="Kargo Notu"
        ...
        <div className="pt-4 border-t border-[#F8F8FA] flex justify-end">
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex items-center gap-2 px-6 py-3 bg-accent text-white rounded-2xl font-black text-sm hover:bg-accent/90 transition-colors disabled:opacity-50 cursor-pointer"
          >
            <Save size={14} />
...
function Field({
  ...
  const cls = 'w-full border border-[#F8F8FA] rounded-xl p-3 text-sm focus:outline-none focus:border-accent bg-white text-gray-900';
  return (
    <div>

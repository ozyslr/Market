...
export function ReviewCard({ review, currentUserId, isSeller, currentUserName }: Props) {
  ...
  return (
    <div className="pb-8 border-b border-gray-200 dark:border-zinc-800 last:border-0 pt-4">
      {/* Başlık */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-black text-gray-900 dark:text-white">{review.userName}</p>
      ...
      <div className="flex flex-wrap items-center gap-1 mb-3">
        {Array.from({ length: 5 }).map((_, i) => (
          <Star
            ...
            className={i < review.rating ? 'text-accent' : 'text-gray-300 dark:text-zinc-700'}
          />
        ))}
        {review.categoryRatings &&
          Object.entries(review.categoryRatings).map(([key, val]) =>
            val && val > 0 ? (
              <span key={key} className="ml-2 text-[9px] font-bold text-gray-400 dark:text-zinc-500 bg-gray-100 dark:bg-zinc-800 px-2 py-0.5 rounded-lg">
      ...
      {/* Yorum metni */}
      <p className="text-sm text-gray-700 dark:text-zinc-300 leading-relaxed font-medium mb-2">
        {displayedComment}
      </p>
      {isLongComment && (
        <button
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          {review.photos.map((photo, i) => (
            <button
              ...
              className="w-20 h-20 rounded-xl overflow-hidden border border-gray-200 dark:border-zinc-800 hover:border-accent transition-colors"
            >
      ...
      <div className="flex items-center gap-4">
        <button
          ...
          className={cn(
            ...
            hasVoted ? 'text-accent' : 'text-gray-400 dark:text-zinc-500 hover:text-accent',
        ...
        {isSeller && !localResponse && (
          <button
            ...
            className="text-[10px] font-black uppercase text-gray-400 dark:text-zinc-500 hover:text-accent transition-colors"
          >
            Yanıtla
          </button>
        )}
      ...
      {showReplyForm && (
        <div className="mt-3 ml-4 flex gap-2">
          <input
            ...
            maxLength={1000}
            className="flex-1 px-3 py-2 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-800 rounded-xl text-xs outline-none focus:border-accent text-gray-900 dark:text-white"
          />
          <button
            onClick={handleReply}
          ...
          <button
            ...
            className="px-3 py-2 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-800 rounded-xl text-xs font-black text-gray-500 dark:text-zinc-400"
          >
            İptal
          </button>
        </div>
      ...
      {localResponse && (
        <div className="mt-3 ml-4 pl-3 border-l-2 border-accent/30 bg-accent/5 rounded-r-xl py-2 pr-3">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[9px] text-gray-400 dark:text-zinc-500 font-bold">
              · {localResponse.createdAt.split('T')[0]}
            </span>
          </div>
          <p className="text-xs font-bold text-gray-700 dark:text-zinc-300">{localResponse.text}</p>
        </div>
      )}

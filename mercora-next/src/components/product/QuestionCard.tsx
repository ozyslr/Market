...
export function QuestionCard({ question, currentUserId, isSeller, onAnswer }: Props) {
  ...
  return (
    <div className="bg-gray-50 dark:bg-zinc-900/60 rounded-2xl p-4 border border-gray-200 dark:border-zinc-800">
      {/* Soru */}
      <div className="flex items-start justify-between gap-4 mb-2">
        <div className="flex items-start gap-2 min-w-0">
          <HelpCircle size={14} className="text-accent mt-0.5 shrink-0" />
          <div className="min-w-0">
            <p className="text-sm font-bold text-gray-900 dark:text-white leading-relaxed break-words">{question.text}</p>
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              <span className="text-[10px] text-gray-400 dark:text-zinc-500 font-bold">
                {question.userName} · {new Date(question.createdAt).toLocaleDateString('tr-TR')}
              </span>
              {question.category && question.category !== 'other' && (
                <span className="text-[8px] font-black uppercase tracking-wider bg-gray-200 dark:bg-zinc-800 text-gray-500 dark:text-zinc-400 px-1.5 py-0.5 rounded">
        ...
        <button
          ...
          className={cn(
            'flex items-center gap-1.5 text-[9px] font-black uppercase transition-colors shrink-0 p-1 rounded hover:bg-gray-200 dark:hover:bg-zinc-800',
      ...
      {localAnswer ? (
        <div className="ml-5 pl-3 border-l-2 border-accent/30 mt-2">
          <p className="text-xs font-bold text-gray-700 dark:text-zinc-300">{localAnswer.text}</p>
          ...
          {!showAnswerForm ? (
            ...
            <div className="ml-5 mt-2 flex gap-2">
              <input
                ...
                maxLength={1000}
                className="flex-1 px-3 py-1.5 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-800 rounded-xl text-xs outline-none focus:border-accent text-gray-900 dark:text-white"
              />
              <button
              ...
              <button
                ...
                className="px-3 py-1.5 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-800 rounded-xl text-xs font-black text-gray-500 dark:text-zinc-400"
              >
                İptal
              </button>
            </div>
        ...
        <p className="ml-5 text-[10px] text-gray-400 dark:text-zinc-600 font-bold italic mt-1">
          Henüz cevaplanmadı
        </p>
      )}
    </div>

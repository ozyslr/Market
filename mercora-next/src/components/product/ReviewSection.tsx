    ...
    return (
      <div className="animate-pulse space-y-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-24 bg-gray-100 dark:bg-zinc-800/50 rounded-2xl" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 mt-6">
      ...
      <div>
        ...
        {!isLoggedIn ? (
          <p className="text-xs font-bold text-gray-500 px-4 py-3 bg-gray-100 dark:bg-zinc-900 rounded-xl">
            Yorum yazmak için <Link href="/auth" className="text-accent underline">giriş yapın</Link>.
          </p>
        ) : hasReviewed ? (
      ...
      <div className="space-y-0">
        {filtered.length === 0 ? (
          <p className="text-center py-8 text-sm text-gray-400 dark:text-zinc-500 font-bold">
      ...
      {paginated.length < filtered.length && (
        <button
          ...
          className="w-full py-4 border border-gray-200 dark:border-zinc-800 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors text-gray-800 dark:text-white"
        >
